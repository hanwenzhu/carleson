...
\inputleannode{classical_carleson}
...
In this blueprint, we prove Theorem \ref{classical_carleson} as a corollary to a further generalization of the polynomial Carleson operator towards doubling metric measure spaces,
Theorem \ref{metric_carleson} below,
which is new and appears in the sibling communication \cite{becker2024carlesonoperatorsdoublingmetric}.
Theorem \ref{metric_carleson} is an axiomatic approach to
...
\begin{equation}
    a\ge 4
\end{equation}
be a natural number that as it gets larger will allow for more general applications of Theorem \ref{metric_carleson}
...
\inputleannode{linearized_metric_carleson}

\begin{remark}
The value of the constant factor $2^{443a^3}$ in Theorems~\ref{metric_carleson}
and~\ref{linearized_metric_carleson} is by no means sharp.
...
An a posteriori byproduct of the Lean formalization of this document is that Theorems~\ref{metric_carleson}
and~\ref{linearized_metric_carleson} remain true with $2^{44a^3}$ instead of $2^{443a^3}$. This is obtained by
...
This section organizes the proof of \Cref{metric_carleson} into sections \ref{thmfromproplinear}, \ref{christsection}, \ref{proptopropprop}, \ref{antichainboundary}, \ref{treesection}, \ref{liphoel}, and \ref{sec-hlm}. These sections are mutually independent except for referring to the statements formulated in the present section. \Cref{thmfromproplinear} proves the main \Cref{metric_carleson}, while sections \ref{christsection}, \ref{proptopropprop}, \ref{antichainboundary}, \ref{treesection}, \ref{liphoel}, and \ref{sec-hlm} each prove one proposition that is stated in the present section. The present section also introduces all definitions used across these sections.

\Cref{global-auxiliary-lemmas} proves some auxiliary lemmas that are used in more than one of the sections 3-9.

Let $a, q$ be given as in \Cref{metric_carleson}.
...
In \Cref{thmfromproplinear}, we prove \Cref{metric_carleson} and \Cref{linearized_metric_carleson}
from the finitary version, \Cref{finitary_carleson} below. Recall
...
\inputleannode{finitary_carleson}
...
In \Cref{christsection}, we prove \Cref{finitary_carleson} using a
bound for a dyadic model formulated in \Cref{discrete_carleson} below.
...
\inputleannode{discrete_carleson}
...
The proof of \Cref{discrete_carleson} is done in \Cref{proptopropprop}
...
\inputleannode{antichain_operator}
...
\inputleannode{forest_operator}

\Cref{metric_carleson} is formulated at the level of generality
...
\inputleannode{holder_van_der_corput}
...
\inputleannode{Finset.measure_biUnion_le_lintegral}

This completes the overview of the proof of \Cref{metric_carleson}.
...
\inputleannode{Θ.finite_and_mk_le_of_le_dist}
...
\inputleannode{Grid.dist_mono}



We also record the following basic estimates for the kernels $K_s$.

\inputleannode{dist_mem_Icc_of_Ks_ne_zero}
...
In this section we prove \Cref{metric_carleson} and \Cref{linearized_metric_carleson}.
...
\inputleannode{continuous_carlesonOperatorIntegrand}



We now prove Theorem \ref{metric_carleson} using Theorem \ref{linearized_metric_carleson}.



We continue with the proof of Theorem \ref{linearized_metric_carleson}, via a series of reductions to simpler lemmas.

Let a measurable function $Q$ with finite range be given.



\inputleannode{R_truncation}



\inputleannode{S_truncation}



\inputleannode{linearized_truncation}
...
\ref{finitary_carleson}, we already fixed in \Cref{overviewsection}
...
\inputleannode{tile_existence}

Choose a grid structure $(\mathcal{D}, c,s)$ with \Cref{grid_existence} and a tile structure for this
grid structure $(\fP,\scI,\fc,\fcc,\pc,\ps)$ with \Cref{tile_existence}.
Applying \Cref{discrete_carleson}, we obtain a Borel set $G'$ in $X$ with $2\mu(G')\leq \mu(G)$ such that for all Borel functions $f:X\to \C$ with $|f|\le \mathbf{1}_F$
we have \eqref{disclesssim}.

\inputleannode{tile_sum_operator}


We use this to prove \Cref{finitary_carleson}.
...
\inputleannode{I1_prop_1}



\inputleannode{cover_by_cubes}


\inputleannode{dyadic_property}
...
\inputleannode{transitive_boundary}




\inputleannode{small_boundary}



\inputleannode{smaller_boundary}


\inputleannode{boundary_measure}
...
\label{subsectiles}
Choose a grid structure $(\mathcal{D}, c, s)$ with \Cref{grid_existence}
...
\inputleannode{frequency_ball_cover}
...
\end{equation}
\inputleannode{Construction.disjoint_frequency_cubes}



\inputleannode{Construction.iUnion_ball_subset_iUnion_Ω₁}
...
Let a grid structure $(\mathcal{D}, c, s)$ and a tile structure $(\fP, \scI, \fc, \fcc)$ for this grid structure be given. In \Cref{subsectilesorg}, we decompose the set $\fP$ of tiles into subsets. Each subset will be controlled by one of three methods. The guiding principle of the decomposition is to be able to apply the forest estimate of \Cref{forest_operator} to the final subsets defined in \eqref{defc5}. This application is done in \Cref{subsecforest}. The miscellaneous subsets along the construction of the forests will either be thrown into exceptional sets, which are defined and controlled in \Cref{subsetexcset}, or will be controlled by the antichain estimate of \Cref{antichain_operator}, which is done in \Cref{subsecantichain}. \Cref{subsec-lessim-aux} contains some auxiliary lemmas needed for the proofs in Subsections \ref{subsecforest}-\ref{subsecantichain}.
...
\inputleannode{exceptional_set}

In \Cref{subsecforest}, we identify each set $\fC_5(k,n,j)$ outside $G'$ as forest and use Proposition
\ref{forest_operator} to prove the following lemma.

\inputleannode{forest_union}

In \Cref{subsecantichain}, we decompose
the complement of the set of tiles in Lemma
\ref{forest_union} and apply the antichain estimate of
\Cref{antichain_operator} to prove the following lemma.

\inputleannode{forest_complement}
...
in Lemmas \ref{first_exception},
\ref{second_exception}, and \ref{third_exception}. Adding up these bounds proves \Cref{exceptional_set}.

The bound for $G_1$ follows from the Vitali covering lemma, \Cref{Finset.measure_biUnion_le_lintegral}.

\inputleannode{first_exception}



We turn to the bound of $G_2$, which relies on the Dyadic Covering \Cref{dense_cover} and the
John-Nirenberg \Cref{john_nirenberg} below.

\inputleannode{dense_cover}


\inputleannode{pairwiseDisjoint_E1}



\inputleannode{dyadic_union}



\inputleannode{john_nirenberg}


\inputleannode{second_exception}



We turn to the set $G_3$.

\inputleannode{top_tiles}



\inputleannode{tree_count}



\inputleannode{boundary_exception}


\inputleannode{third_exception}
...
Before proving \Cref{forest_union} and \Cref{forest_complement}, we collect some useful properties of $\lesssim$.

\inputleannode{smul_mono}



\inputleannode{smul_C2_1_2}



\inputleannode{wiggle_order_11_10}
...
\inputleannode{ordConnected_tilesAt}



\inputleannode{ordConnected_C}



\inputleannode{ordConnected_C1}



\inputleannode{ordConnected_C2}



\inputleannode{ordConnected_C3}



\inputleannode{ordConnected_C4}



\inputleannode{ordConnected_C5}



\inputleannode{dens1_le_dens'}


\inputleannode{dens1_le}
...
\inputleannode{URel.eq}




\inputleannode{equivalenceOn_urel}
...
\inputleannode{C6_forest}


\inputleannode{forest_geometry}


\inputleannode{forest_convex}



\inputleannode{forest_separation}



\inputleannode{forest_inner}




\inputleannode{forest_stacking}


We now turn to the proof of \Cref{forest_union}.
...
\inputleannode{antichain_decomposition}



\inputleannode{iUnion_L0'}



\inputleannode{antichain_L2}



\inputleannode{antichain_L1}



We now turn to the proof of \Cref{forest_complement}.
...
and functions $f$, $g$ as in \Cref{antichain_operator} be given.
...
\Cref{Tile.correlation_le} of
the $TT^*$ correlation between two tile operators.
\Cref{Tile.correlation_le} will be proven in
\Cref{sec-tile-operator}.

The summation of the contributions of these individual correlations will require a
geometric \Cref{Antichain.tile_count} counting the relevant tile pairs.
\Cref{Antichain.tile_count} will be proven in Subsection
\ref{subsec-geolem}.
...
\inputleannode{tile_disjointness}
...
\inputleannode{maximal_bound_antichain}
...
\inputleannode{dens2_antichain}



\inputleannode{dens1_antichain}


The following basic $TT^*$ estimate will be proved in \Cref{sec-tile-operator}.
\inputleannode{Tile.correlation_le}

The following lemma will be proved in \Cref{subsec-geolem}.
\inputleannode{Antichain.tile_count}

From these lemmas it is easy to prove \Cref{antichain_operator}.
...
\inputleannode{Tile.range_support}
...
We now prove \Cref{Tile.correlation_le}.
...
\inputleannode{Antichain.tile_reach}
...
\inputleannode{Antichain.stack_density}



\inputleannode{Antichain.local_antichain_density}


\inputleannode{Antichain.global_antichain_density}
...
We turn to the proof of \Cref{Antichain.tile_count}.
...
Fix a forest $(\fU, \fT)$. The main result of this subsection is \Cref{TileStructure.Forest.pointwise_tree_estimate}, we begin this section with some definitions necessary to state the lemma.
...
$$
\inputleannode{TileStructure.Forest.convex_scales}
...
\inputleannode{TileStructure.Forest.biUnion_𝓙}
...
\inputleannode{TileStructure.Forest.pointwise_tree_estimate}



\inputleannode{TileStructure.Forest.first_tree_pointwise}



\inputleannode{TileStructure.Forest.second_tree_pointwise}



\inputleannode{TileStructure.Forest.third_tree_pointwise}
...
\inputleannode{TileStructure.Forest.tree_projection_estimate}

Below, we deduce \Cref{TileStructure.Forest.tree_projection_estimate} from \Cref{TileStructure.Forest.pointwise_tree_estimate} and the following estimates for the operators in \Cref{TileStructure.Forest.pointwise_tree_estimate}.

\inputleannode{TileStructure.Forest.nontangential_operator_bound}

\inputleannode{TileStructure.Forest.boundary_operator_bound}
...
\inputleannode{TileStructure.Forest.boundary_overlap}
...
\inputleannode{TileStructure.Forest.density_tree_bound1}

Below, we deduce this lemma from \Cref{TileStructure.Forest.tree_projection_estimate} and the following two estimates controlling the size of support of the operator and its adjoint.

\inputleannode{TileStructure.Forest.local_dens1_tree_bound}

\inputleannode{TileStructure.Forest.local_dens2_tree_bound}
...
The main result of this subsection is the almost orthogonality estimate for operators associated to distinct trees in a forest in \Cref{TileStructure.Forest.correlation_separated_trees} below. We will deduce it from Lemmas \ref{TileStructure.Forest.correlation_distant_tree_parts} and \ref{TileStructure.Forest.correlation_near_tree_parts}, which are proven in Subsections \ref{subsec-big-tiles} and \ref{subsec-rest-tiles}, respectively. Before stating it, we introduce some relevant notation.
...
\inputleannode{TileStructure.Forest.adjoint_tile_support1}



\inputleannode{TileStructure.Forest.adjoint_tree_estimate}
...
$$
\inputleannode{TileStructure.Forest.adjoint_tree_control}
...
\inputleannode{TileStructure.Forest.correlation_separated_trees}



\inputleannode{TileStructure.Forest.correlation_distant_tree_parts}
    \inputleannode{TileStructure.Forest.correlation_near_tree_parts}

In the proofs of both lemmas, we will need the following observation.

\inputleannode{TileStructure.Forest.𝔗_subset_𝔖₀}



To simplify the notation, we will write at various places throughout the proof of Lemmas \ref{TileStructure.Forest.correlation_distant_tree_parts} and \ref{TileStructure.Forest.correlation_near_tree_parts} for a subset $\fC \subset \fP$
$$
...
\Cref{TileStructure.Forest.correlation_distant_tree_parts} follows from the van der Corput estimate in \Cref{holder_van_der_corput}. We apply this proposition in \Cref{subsubsec-van-der-corput}. To prepare this application, we first, in \Cref{subsubsec-pao}, construct a suitable partition of unity, and show then, in \Cref{subsubsec-holder-estimates} the H\"older estimates needed to apply \Cref{holder_van_der_corput}.
...
\label{subsubsec-pao}
    ...
    \inputleannode{TileStructure.Forest.union_𝓙₅}
    ...
    \inputleannode{TileStructure.Forest.sum_χ}

    In the proof, we will use the following auxiliary lemma.

    \inputleannode{TileStructure.Forest.moderate_scale_change}
...
\label{subsubsec-holder-estimates}
    ...
    \inputleannode{TileStructure.Forest.holder_correlation_tree}
    ...
    \inputleannode{TileStructure.Forest.holder_correlation_tile}
    ...
    \inputleannode{TileStructure.Forest.limited_scale_impact}




    \inputleannode{TileStructure.Forest.local_tree_control}



    \inputleannode{TileStructure.Forest.scales_impacting_interval}



    \inputleannode{TileStructure.Forest.global_tree_control1_edist_left}



    \inputleannode{TileStructure.Forest.global_tree_control2}
...
\label{subsubsec-van-der-corput}
    \inputleannode{TileStructure.Forest.lower_oscillation_bound}



    Now we are ready to prove \Cref{TileStructure.Forest.correlation_distant_tree_parts}.
...
\subsection{Proof of The Remaining Tiles Lemma}
    ...
    \Cref{TileStructure.Forest.correlation_near_tree_parts} follows from the following key estimate.

    \inputleannode{TileStructure.Forest.bound_for_tree_projection}

    We prove this lemma below. First, we deduce \Cref{TileStructure.Forest.correlation_near_tree_parts}.



    We need two more auxiliary lemmas before we prove \Cref{TileStructure.Forest.bound_for_tree_projection}.

    \inputleannode{TileStructure.Forest.thin_scale_impact}



    \inputleannode{TileStructure.Forest.square_function_count}
...
\subsection{Forests}
In this subsection, we complete the proof of \Cref{forest_operator} from the results of the previous subsections.
...
\inputleannode{TileStructure.Forest.rowDecomp}
...
\end{equation*}
as in \Cref{TileStructure.Forest.rowDecomp}. To save some space in the proofs of the remaining lemmas in this section we will write
    $$
    ...
    $$
\inputleannode{TileStructure.Forest.row_bound}
...
\Cref{forest_operator}.

\inputleannode{TileStructure.Forest.row_correlation}
...
\inputleannode{TileStructure.Forest.pairwiseDisjoint_rowSupport}



Now we prove \Cref{forest_operator}.
...
\inputleannode{support_holderApprox_subset}





We turn to the proof of \Cref{holder_van_der_corput}.
...
\inputleannode{MeasureTheory.eLpNorm_pow_eq_distribution}


The following lemma will be used to define $M$ in the proof of \Cref{Finset.measure_biUnion_le_lintegral}.
\inputleannode{covering_separable_space}



We turn to the proof of \Cref{Finset.measure_biUnion_le_lintegral}.


\section{Two-sided Metric Space Carleson}

We prove a variant of \Cref{metric_carleson} for a two-sided Calder\'on--Zygmund kernel on the doubling metric measure space $(X,\rho,\mu,a)$, i.e. a one-sided Calder\'on--Zygmund kernel $K$ which additionally satisfies for all $x,x',y\in X$ with $x\neq y$ and $2\rho(x,x') \leq \rho(x,y)$,
\begin{equation}
...
\end{equation}
\inputleannode{two_sided_metric_carleson}

For the remainder of this chapter, fix an integer $a\ge 4$, a doubling metric measure space $(X,\rho,\mu,a)$ and a two-sided Calder\'on--Zygmund kernel $K$ as in \Cref{two_sided_metric_carleson}.

The following lemma is proved in \Cref{subsec-cotlar}.
\inputleannode{nontangential_from_simple}



The proof of \Cref{nontangential_from_simple} relies on the following auxiliary lemma which is proved in \Cref{subsec-CZD}.
\inputleannode{czOperator_weak_1_1}
Throughout \Cref{subsec-CZD} and \Cref{subsec-cotlar}, for any measurable bounded function $w: X \to \C$, let $Mw: X \to [0, \infty)$ denote the corresponding Hardy--Littlewood maximal function defined in \Cref{Finset.measure_biUnion_le_lintegral}.
Note: In some cases, a stronger formalization of \Cref{Finset.measure_biUnion_le_lintegral} is used, where $w$ is not necessarily bounded. In particular it is applied to $T_rg$.
Apart from \Cref{Finset.measure_biUnion_le_lintegral}, \Cref{subsec-CZD} and \Cref{subsec-cotlar} have no dependencies in the previous chapters.
...
\inputleannode{geometric_series_estimate}


\inputleannode{estimate_x_shift}


\inputleannode{cotlar_control}



\inputleannode{cotlar_set_F₁}



\inputleannode{cotlar_estimate}
...
\inputleannode{simple_nontangential_operator}
...
\inputleannode{small_annulus_right}


\inputleannode{nontangential_operator_boundary}
...
The following lemma is Theorem 3.1(b) in \cite{stein-book}. The proof uses \Cref{Finset.measure_biUnion_le_lintegral}.
%fixme: Incorporate this into Hardy-Littlewood?
\inputleannode{maximal_theorem}


\inputleannode{lebesgue_differentiation}


% This is in Mathlib.
\inputleannode{Pairwise.countable_of_isOpen_disjoint}
...
\begin{equation*}
    B_{n,j} = B(x_j, nr_j).
\end{equation*}
\inputleannode{ball_covering}


Most of the next lemma and its proof is taken from Theorem 4.2 in \cite{stein-book}.
\inputleannode{encard_czBall3_le}



We use \Cref{encard_czBall3_le} to prove \Cref{czOperator_weak_1_1}. For the remainder of this section, let $f:X\to\C$, $r>0$ and $\alpha>0$ as in \Cref{czOperator_weak_1_1}.
...
Using \Cref{encard_czBall3_le} for $f$ and $\alpha'$, we obtain the decomposition
\begin{equation*}
    f=g+b=g+\sum_j b_j
\end{equation*}
...
\inputleannode{estimate_good}



\inputleannode{estimate_bad_partial}



\inputleannode{distribution_czOperatorBound}




\inputleannode{estimate_bad}
...
we establish convergence on a suitable dense subclass of functions. We choose smooth functions as subclass, the convergence is stated in \Cref{fourierConv_ofTwiceDifferentiable} and proved in \Cref{10smooth}.
In the second step, one controls the relevant error of approximating a general function by a function in
the subclass. This is stated in \Cref{control_approximation_effect} and proved
in \Cref{10difference}.
The proof relies on a bound on the real Carleson maximal operator stated in \Cref{rcarleson} and proved in \Cref{10carleson}, which involves showing that the real line fits into the setting of \Cref{overviewsection}.
This latter proof refers to the two-sided variant of the Carleson \Cref{two_sided_metric_carleson}. Two assumptions in \Cref{metric_carleson} require more work. The boundedness of the operator $T_r$ defined in \eqref{def-T-r} is established in \ref{Hilbert_strong_2_2}. This lemma is proved in \Cref{10hilbert}.
The cancellative property is verified by \Cref{van_der_Corput}, which is proved in \Cref{10vandercorput}.
Several further auxiliary lemmas are stated and proved in \Cref{10classical}, the proof of one of these auxiliary lemmas, \Cref{spectral_projection_bound}, is done in \Cref{10projection}.
...
\end{equation}
denote the constant from \Cref{two_sided_metric_carleson}.
Define
\begin{equation}
...
\inputleannode{close_smooth_approx_periodic}



We prove in \Cref{10smooth}:
\inputleannode{fourierConv_ofTwiceDifferentiable}

We prove in \Cref{10difference}:
\inputleannode{control_approximation_effect}

We are now ready to prove \Cref{classical_carleson}. We first prove a version with explicit exceptional sets.

\inputleannode{exceptional_set_carleson}
...
The proof of \Cref{control_approximation_effect} will
use the following \Cref{rcarleson}, which itself is proven
 in \Cref{10carleson} as an application of
 \Cref{metric_carleson}.
\inputleannode{rcarleson}


One of the main assumptions of \Cref{two_sided_metric_carleson}, concerning the operator $T_r$ defined in \eqref{def-T-r}, is verified by the following lemma, which is proved in \Cref{10hilbert}.
\inputleannode{Hilbert_strong_2_2}

The next lemma will be used to verify that the collection $\Mf$ of modulation functions in our application of \Cref{metric_carleson} satisfies the condition \eqref{eq-vdc-cond}.
It is proved in \Cref{10vandercorput}.

\inputleannode{van_der_Corput}


We close this section with five lemmas that are used
across the following subsections.

\inputleannode{dirichletKernel_eq}




\inputleannode{lower_secant_bound'}


The following lemma will be proved in \Cref{10projection}.

\inputleannode{spectral_projection_bound}

\inputleannode{Hilbert_kernel_bound}


\inputleannode{Hilbert_kernel_regularity}
...
\label{10smooth}
\inputleannode{fourierCoeffOn_of_hasDerivAt}


\inputleannode{hasSum_fourier_series_of_summable}
...
\inputleannode{modulated_averaged_projection}


\inputleannode{Function.Periodic.intervalIntegral_add_eq}




\inputleannode{young_convolution}
...
\inputleannode{integrable_bump_convolution}



\inputleannode{continuous_dirichletApprox}





We now prove \Cref{Hilbert_strong_2_2}.
...
This subsection proves \Cref{spectral_projection_bound}
...
\inputleannode{Dirichlet_Hilbert_diff}



\inputleannode{partialFourierSum_bound}



%fixme: Put this somewhere else?
\inputleannode{carlesonOperatorReal_measurable}
...
\end{equation}
on the real line $\R$.
\inputleannode{instProperSpaceReal}

\inputleannode{Real.ball_eq_Ioo}

We consider the Lebesgue measure $\mu$ on $\R$.
\inputleannode{instIsAddHaarMeasureVolume}

\inputleannode{Real.volume_ball}


\inputleannode{MeasureTheory.InnerProductSpace.IsDoubling}



The preceding four lemmas show that $(\R, \rho, \mu, 4)$ is a doubling metric measure space. Indeed, we even show
that $(\R, \rho, \mu, 1)$ is a doubling metric measure space, but we may relax the estimate in \Cref{MeasureTheory.InnerProductSpace.IsDoubling} to conclude that $(\R, \rho, \mu, 4)$
...
\inputleannode{instFunctionDistancesReal}



\inputleannode{oscillation_control}



\inputleannode{frequency_monotone}



\inputleannode{frequency_ball_doubling}



\inputleannode{frequency_ball_growth}



\inputleannode{integer_ball_cover}




\inputleannode{real_van_der_Corput}
