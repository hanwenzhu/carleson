...
/--
Let $\fp,\fp'\in \mathfrak{A}$. If there exists an $x\in X$ with $x\in E(\fp)\cap E(\fp')$, then
$\fp= \fp'$.
...
@[blueprint
  "tile disjointness"
  (proof := /--
  Let $\fp,\fp'$ and $x$ be given. Assume without loss of generality that $\ps(\fp)\le \ps(\fp')$. As
  we have $x\in E(\fp)\subset \scI(\fp)$ and $x\in E(\fp')\subset \scI(\fp')$ by Definition
  \ref{defineep}, we conclude for $i=1,2$ that $\tQ(x)\in\fc(\fp)$ and
  $\tQ(x)\in\fc(\fp')$. By \ref{eq-freq-dyadic} we have $\fc(\fp')\subset \fc(\fp)$.
  By Definition \ref{straightorder}, we conclude $\fp\le \fp'$. As $\mathfrak{A}$ is
  an antichain, we conclude $\fp=\fp'$. This proves the lemma.
  -/)
  (latexEnv := "lemma")]
...
@[blueprint
  ...
  (proof := /--
  Fix $x\in X$. By `tile_disjointness`, there is at most one $\fp \in \mathfrak{A}$ such that
  $T_{\fp} f(x)$ is not zero. If there is no such $\fp$, the estimate \ref{hlmbound}
  follows.
  
  Assume there is such a $\fp$. By definition of $T_{\fp}$ we have $x\in E(\fp)\subset \scI(\fp)$ and
  by the squeezing property \ref{eq-vol-sp-cube} $$\begin{equation}
  \label{eqtttt0}
      \rho(x, \pc(\fp))\le 4D^{\ps(\fp)}\, .
  \end{equation}$$
  
  Let $y\in X$ with $K_{\ps(\fp)}(x,y)\neq 0$. By Definition \ref{defks} of $K_{\ps(\fp)}$ we
  have $$\begin{equation}
  \label{supp-Ks1}
     \frac{1}{4} D^{\ps(\fp)-1}
     \leq \rho(x,y) \leq \frac{1}{2} D^{\ps(\fp)}\, .
  \end{equation}$$ The triangle inequality with \ref{eqtttt0} and \ref{supp-Ks1}
  implies $$\begin{equation}
      \rho(\pc(\fp),y) < 8D^{\ps(\fp)}\, .
  \end{equation}$$ Using the kernel bound \ref{eqkernel-size} and the lower bound in
  \ref{supp-Ks} we obtain $$\begin{equation}
  |K_{\ps(\fp)}(x,y)|\le \frac{2^{a^3}}{\mu(B(x,\frac 14 D^{{\ps(\fp)}-1}))}\, .
  \end{equation}$$ Using $D=2^{100a^2}$ and the doubling property \ref{doublingx}
  $5 +100a^2$ times estimates the last display by $$\begin{equation}
  \le \frac{2^{5a+101a^3}}{\mu(B(x, 8D^{\ps(\fp)}))}\,
  \end{equation}$$ which, thanks to the closeness of the points $x$ and $\pc(\fp)$ shown in
  \ref{eqtttt0}, is in turn bounded by $$\begin{equation}
  \le \frac{2^{6a+101a^3}}{\mu(B(\pc(\fp), 8D^{\ps(\fp)}))}\, .
  \end{equation}$$ Using that $|e(\mfa)|$ is bounded by $1$ for every $\mfa\in \Mf$, we estimate with
  the triangle inequality and the above information $$\begin{equation}
    | T_{\fp} f(x)|
      \le \frac{2^{6a+101 a^3}}{\mu(B(\pc(\fp), 8D^{\ps(\fp)}))} \int _{\mu(B(\pc(\fp), 8D^{\ps(\fp)}))} |f(y)|\, dy
  \end{equation}$$ This together with $a\ge 4$ proves the Lemma.
  -/)
  (latexEnv := "lemma")]
...
@[blueprint
  "dens2 antichain"
  (proof := /--
  We have $f=\mathbf{1}_Ff$. Using Hölder's inequality, we obtain for each $x\in B'$ and each
  $B'\in \mathcal{B}$ using $1<\tilde{q}\le 2$ $$\begin{equation}
  ...
  \end{equation}$$ Taking the maximum over all $B'$ containing $x$, we obtain $$\begin{equation}
   \label{eqttt1}
      M_{\mathcal{B}}|f|\le
      M_{\mathcal{B},\frac {2{\tilde{q}}}{3{\tilde{q}}-2} } |f|
      \dens_2(\mathfrak{A})^{\frac 1{\tilde{q}}-\frac 12}\, .
  \end{equation}$$ We have with `Finset.measure_biUnion_le_lintegral` $$\begin{equation}
  \left\|M_{\mathcal{B}, \frac {2{\tilde{q}}}{3{\tilde{q}}-2}} f\right\|_2\le 2^{2a}(3\tilde{q}-2)(2\tilde{q}-2)^{-1}\|f\|_2\, .
  \end{equation}$$ Using $1<\tilde{q}\le 2$ estimates the last display by $$\begin{equation}
  \label{eqttt2}
   2^{2a+2} (\tilde{q}-1)^{-1} \|f\|_2\, .
  \end{equation}$$ We obtain with Cauchy-Schwarz and then `maximal_bound_antichain` $$\begin{equation}
  ...
  \end{equation}$$ With \ref{eqttt1} and \ref{eqttt2} we can estimate the last
  display by $$\begin{equation}
      \le 2^{102a^3+2a+2}(\tilde{q}-1)^{-1} \|g\|_2 \|f\|_2\dens_2(\mathfrak{A})^{\frac 1{\tilde{q}}-\frac 12}
  \end{equation}$$ Using $a\ge 4$ and $(\tilde q - 1)^{-1} = (q+1)/(q-1) \le 3(q-1)^{-1}$ proves the
  lemma.
  -/)
  (latexEnv := "lemma")]
