...
@[blueprint
  "dens1-antichain"
  (title := "dens1 antichain")
  (statement := /-- Set $p:=4a^4$. We have
    ...
    \end{equation} -/)
  (proof := /-- We write for the expression inside the absolute values on the left-hand side of
    \eqref{eqttt3}
    \begin{equation}
    ...
    \begin{equation}
        ...
        \mathfrak{A}(\fp):=\{\fp'\in\mathfrak{A}: \ps(\fp')\leq \ps(\fp) \land \scI(\fp') \subset
        B(\fp)\}.
    \end{equation}
    ...
    \begin{equation}\label{def-hp}
        ...
        {(1+d_{\fp'}(\fcc(\fp'), \fcc(\fp))^{-1/(2a^2+a^3)}}(\mathbf{1}_{E(\fp')}|g|)(y')\,
        d\mu(y')\,.
    \end{equation}
    
    Note that $p>4$ since $a\ge 4$. Let $p'$ be the dual exponent of $p$, satisfying $1/p+1/p'=1$.
    We estimate $h(\fp)$ as defined in \eqref{def-hp} with H\"older using $|g|\le \mathbf{1}_G$ and
    $E(\fp')\subset B(\fp)$ by
    
    \begin{equation}
        \frac{\|g\mathbf{1}_{B(\fp)}\|_{p'}}{\mu(B(\fp))}
        \Big\|\sum_{\fp\in\mathfrak{A}(\fp)}(1+d_{\fp}(\fcc(\fp),
        \fcc(\fp'))^{-1/(2a^2+a^3)}\mathbf{1}_{E(\fp)}\mathbf{1}_G\Big\|_{p}\, .
    \end{equation}
    ...
    \end{equation}
    With this estimate of $h(\fp)$, using $E(\fp)\subset B(\fp)$ by construction of $B(\fp)$, we
    estimate \eqref{eqtts3} by
    ...
    \le 2^{232a^3+11a + 1}
     {\dens_1(\mathfrak{A})^{\frac 1p}} \int |g|(y)(M_{\mathcal{B}', p'}g)(y) \, dy\,.
             \end{equation}
    Applying Cauchy-Schwarz and using \Cref{Hardy-Littlewood} and $1<p'<\frac 32$ estimates the last
    display by
    \begin{equation}
...
@[blueprint
  "antichain-operator"
  (title := "antichain operator")
  (statement := /-- For any antichain $\mathfrak{A} $ and for all $f:X\to \C$ with $|f|\le
    \mathbf{1}_F$ and all $g:X\to\C$ with $|g| \le \mathbf{1}_G$
    \begin{equation} \label{eq-antiprop}
        |\int \overline{g(x)} \sum_{\fp \in \mathfrak{A}} T_{\fp} f(x)\, d\mu(x)|
    \end{equation}
    \begin{equation}
        \le \frac{2^{117a^3}}{q-1} \dens_1(\mathfrak{A})^{\frac
        {q-1}{8a^4}}\dens_2(\mathfrak{A})^{\frac 1{q}-\frac 12} \|f\|_2 \|g\|_2\, .
    \end{equation} -/)
  (proof := /-- [Proof of \Cref{antichain-operator}]
    ...
    \end{equation}
     \begin{equation}
        \le 2^{117a^3}(q-1)^{-1} \dens_1(\mathfrak{A})^{\frac {q-1}{2p}}\dens_2(\mathfrak{A})^{\frac
        1{q}-\frac 12} \|f\|_2\|g\|_2\, .
    \end{equation}
