...
@[blueprint
  "dens-compare"
  (title := "dens compare")
  (statement := /-- We have for every $k\ge 0$ and $\fP'\subset \fP(k)$
    \begin{equation}
        \dens_1(\fP')\le \dens_k'(\fP')\, .
    \end{equation} -/)
  (proof := /-- It suffices to show that for all $\fp'\in \fP'$
    ...
    $\fp''\in \fP'$ with $\scI(\fp')\subset \scI(\fp'')$. By assumption on $\fP'$, we have $\fp''\in
    \fP(k)$ and there exists
    $J\in \mathcal{D}$ with
       $\scI(\fp'')\subset J$ and
       \begin{equation}
...
@[blueprint
  "C-dens1"
  (title := "C dens1")
  (statement := /-- For each set $\mathfrak{A} \subset \mathfrak{C}(k,n)$, we have
    $$
        \dens_1(\mathfrak{A}) \le 2^{4a}2^{-n+1}\,.
    $$ -/)
  (proof := /-- We have by \Cref{dens-compare} that
    $\dens_1(\mathfrak{A}) \le \dens_k'(\mathfrak{A})$. Since $\mathfrak{A} \subset \fC(k,n)$, it
    follows from monotonicity of suprema and the definition \eqref{eq-densdef} that
    $
