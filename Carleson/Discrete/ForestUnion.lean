...
@[blueprint
  "P convex"
  (proof := /--
  Suppose that $\fp \le \fp' \le \fp''$ and $\fp, \fp'' \in \fP(k)$. By \ref{eq-defPk} we
  have $\scI(\fp), \scI(\fp'') \in \mathcal{C}(G,k)$, so there exists by \ref{muhj1} some
  $J \in \mathcal{D}$ with $$\scI(\fp') \subset \scI(\fp'') \subset J$$ and
  $\mu(G \cap J) > 2^{-k-1} \mu(J)$. Thus \ref{muhj1} holds for $\scI(\fp')$. On the other
  hand, by \ref{muhj2}, there exists no $J \in \mathcal{D}$ with $\scI(\fp) \subset J$ and
  $\mu(G \cap J) > 2^{-k} \mu(J)$. Since $\scI(\fp) \subset \scI(\fp')$, this implies that
  \ref{muhj2} holds for $\scI(\fp')$. Hence $\scI(\fp') \in \mathcal{C}(G,k)$, and therefore
  by \ref{eq-defPk} $\fp' \in \fP(k)$.
  -/)
  (latexEnv := "lemma")]
...
@[blueprint
  "C convex"
  (proof := /--
  Let $\fp \le \fp' \le \fp''$ with $\fp, \fp'' \in \fC(k,n)$. Then, in particular,
  $\fp, \fp'' \in \fP(k)$, so, by `ordConnected_tilesAt`, $\fp' \in \fP(k)$. Next, we show that if
  $\fq \le \fq' \in \fP(k)$ then $\dens'_k(\{\fq\}) \ge \dens_k'(\{\fq'\})$. If $\fp \in \fP(k)$ and
  $\lambda \ge 2$ with $\lambda \fq' \lesssim \lambda \fp$, then it follows from $\fq \le \fq'$,
  \ref{eq-sc1} of `wiggle_order_11_10` and transitivity of $\lesssim$ that
  $\lambda \fq \lesssim \lambda \fp$. Thus the supremum in the definition
  \ref{eq-densdef} of $\dens_k'(\{\fq\})$ is over a superset of the set the supremum in
  the definition of $\dens_k'(\{\fq'\})$ is taken over, which shows
  $\dens'_k(\{\fq\}) \ge \dens_k'(\{\fq'\})$. From $\fp' \le \fp''$, $\fp'' \in \fC(k,n)$ and
  \ref{def-cnk} it then follows that
  $$2^{4a}2^{-n} < \dens_k'(\{\fp''\}) \le \dens_k'(\{\fp'\})\,.$$ Similarly, it follows from
  $\fp \le \fp'$, $\fp \in \fC(k,n)$ and \ref{def-cnk} that
  $$\dens_k'(\{\fp'\}) \le \dens_k'(\{\fp\}) \le 2^{4a}2^{-n+1}\,.$$ Thus $\fp' \in \fC(k,n)$.
  -/)
  (latexEnv := "lemma")]
...
@[blueprint
  "C1 convex"
  (proof := /--
  Let $\fp\le\fp'\le\fp''$ with $\fp, \fp'' \in \fC_1(k,n,j)$. By `ordConnected_C` and the inclusion
  $\fC_1(k,n,j) \subset \fC(k,n)$, which holds by definition \ref{defcnkj}, we have
  $\fp' \in \fC(k,n)$. By \ref{eq-sc1} and transitivity of $\lesssim$ we have that
  $\fq \le \fq'$ and $100 \fq' \lesssim \mathfrak{m}$ imply $100 \fq \lesssim \mathfrak{m}$. So, by
  \ref{defbfp}, $\mathfrak{B}(\fp'') \subset \mathfrak{B}(\fp') \subset \mathfrak{B}(\fp)$.
  Consequently, by \ref{defcnkj}
  $$2^j \le |\mathfrak{B}(\fp'')|\le |\mathfrak{B}(\fp')| \le |\mathfrak{B}(\fp)| < 2^{j+1}\,,$$ thus
  $\fp' \in \fC_1(k,n,j)$.
  -/)
  (latexEnv := "lemma")]
...
@[blueprint
  "C2 convex"
  (proof := /--
  Let $\fp \le \fp' \le \fp''$ with $\fp, \fp'' \in \fC_2(k,n,j)$. By \ref{eq-C2-def}, we
  have $$\begin{equation*}
           \fC_2(k,n,j) \subset \fC_1(k,n,j)\ .
  \end{equation*}$$ Combined with `ordConnected_C1`, it follows that $\fp' \in \fC_1(k,n,j)$. If
  $\fp=\fp'$ the statement is trivially true, otherwise suppose that $\fp' \notin \fC_2(k,n,j)$. By
  \ref{eq-C2-def}, this implies that there exists $0 \le l' \le Z(n+1)$ with
  $\fp' \in \fL_1(k,n,j,l')$. By the definition \ref{eq-L1-def} of $\fL_1(k,n,j,l')$, this
  implies that $\fp'$ is minimal with respect to $\le$ in
  $\fC_1(k,n,j) \setminus \bigcup_{l < l'} \fL_1(k,n,j,l)$. Since $\fp\le\fp'$ and
  $\fp\in\fC_2(k,n,j)$, $\fp=\fp'$, a contradiction.
  -/)
  (latexEnv := "lemma")]
...
@[blueprint
  "C3 convex"
  (proof := /--
  Let $\fp \le \fp' \le \fp''$ with $\fp, \fp'' \in \fC_3(k,n,j)$. By \ref{eq-C3-def} and
  `ordConnected_C2` it follows that $\fp' \in \fC_2(k,n,j)$. By \ref{eq-C3-def} and
  \ref{eq-L2-def}, there exists $\fu \in \fU_1(k,n,j)$ with $2\fp'' \lesssim \fu$ and
  $\scI(\fp'') \subsetneq \scI(\fu)$. From $\fp' \le \fp''$, \ref{eq-sc1} and transitivity of
  $\lesssim$ we then have $2\fp' \lesssim \fu$ and $\scI(\fp') \subsetneq \scI(\fu)$, so
  $\fp' \in \fC_3(k,n,j)$.
  -/)
  (latexEnv := "lemma")]
...
@[blueprint
  "C4 convex"
  (proof := /--
  The proof is entirely analogous to `ordConnected_C2`, substituting $\fC_4$ for $\fC_2$, $\fC_3$ for
  $\fC_1$ and $\fp'\le\fp''$ for $\fp\le\fp'$.
  -/)
  (latexEnv := "lemma")]
...
@[blueprint
  "C5 convex"
  (proof := /--
  Let $\fp \le \fp' \le\fp''$ with $\fp, \fp'' \in \fC_5(k,n,j)$. Then $\fp, \fp'' \in \fC_4(k,n,j)$
  by \ref{defc5}, and thus by `ordConnected_C4` $\fp' \in \fC_4(k,n,j)$. It suffices to show
  that if $\fp' \in \fL_4(k,n,j)$ then $\fp \in \fL_4(k,n,j)$ by contraposition; this is true by
  \ref{eq-L4-def} and $\fp\le\fp'$.
  -/)
  (latexEnv := "lemma")]
...
/--
If $\fu \sim \fu'$, then $\scI(u) = \scI(u')$ and $$\begin{equation*}
...
@[blueprint
  "relation geometry"
  (proof := /--
  Let $\fu, \fu' \in \fU_2(k,n,j)$ with $\fu \sim \fu'$. If $\fu = \fu'$ then the conclusion of the
  Lemma clearly holds. Else, there exists $\fp \in \fC_1(k,n,j)$ such that $\scI(\fp) \ne \scI(\fu)$
  and $2 \fp \lesssim \fu$ and $10 \fp \lesssim \fu'$. Using `smul_mono` and \ref{eq-sc2} of
  `wiggle_order_11_10`, we deduce that $$\begin{equation}
          \label{eq-Fefferman-trick0}
          100 \fp\lesssim 100 \fu\,, \qquad 100 \fp \lesssim 100\fu'\,.
  \end{equation}$$ Now suppose that
  $B_{\fu}(\fcc(\fu), 100) \cap B_{\fu'}(\fcc(\fu'), 100) = \emptyset$. Then we have
  $\mathfrak{B}(\fu) \cap \mathfrak{B}(\fu') = \emptyset$, by the definition \ref{defbfp} of
  $\mathfrak{B}$ and the definition \ref{wiggleorder} of $\lesssim$, but also
  $\mathfrak{B}(\fu) \subset \mathfrak{B}(\fp)$ and $\mathfrak{B}(\fu') \subset \mathfrak{B}(\fp)$, by
  \ref{defbfp}, \ref{wiggleorder} and
  \ref{eq-Fefferman-trick0}. Hence,
  $$|\mathfrak{B}(\fp)| \geq |\mathfrak{B}(\fu)| + |\mathfrak{B}(\fu')| \geq 2^{j} + 2^j = 2^{j+1}\,,$$
  which contradicts $\fp \in \fC_1(k,n,j)$. Therefore we must have $$\begin{equation*}
          B_{\fu}(\fcc(\fu), 100) \cap B_{\fu'}(\fcc(\fu'), 100) \ne \emptyset\, .
  \end{equation*}$$

  It follows from $2\fp \lesssim \fu$ and $10\fp \lesssim \fu'$ that $\scI(\fp) \subset \scI(\fu)$ and
  $\scI(\fp) \subset \scI(\fu')$. By \ref{dyadicproperty}, it follows that
  $\scI(\fu)$ and $\scI(\fu')$ are nested. Combining this with the conclusion of the last paragraph
  and definition \ref{defunkj} of $\fU_1(k,n,j)$, we obtain that $\scI(\fu) = \scI(\fu')$.
  -/)
  (latexEnv := "lemma")]
...
@[blueprint
  ...
  (proof := /--
  Reflexivity holds by definition. For transitivity, suppose that $$\begin{equation*}
          \fu, \fu', \fu'' \in \fU_1(k,n,j)
  \end{equation*}$$ and $\fu \sim \fu'$, $\fu' \sim \fu''$. By `URel.eq`, it follows that
  $\scI(\fu) =\scI(\fu') = \scI(\fu'')$, that there exists $$\begin{equation*}
          \mfa \in B_{\fu}(\fcc(\fu), 100) \cap B_{\fu'}(\fcc(\fu'), 100)
  \end{equation*}$$ and that there exists $$\begin{equation*}
          \mfb \in B_{\fu'}(\fcc(\fu'), 100) \cap B_{\fu''}(\fcc(\fu''), 100)\, .
  \end{equation*}$$ If $\fu = \fu'$, then $\fu \sim \fu''$ holds by assumption. Else, there exists by
  the definition of $\sim$ some $\fp \in \mathfrak{T}_1(\fu)$ with $10\fp\lesssim \fu'$. Then we have
  $2\fp \lesssim \fu$ and $\fp \ne \fu$ by definition of $\mathfrak{T}_1(\fu)$, so
  $4 \fp \lesssim 500 \fu$ by \ref{eq-sc3}. For $q \in B_{\fu''}(\fcc(\fu''), 1)$ it follows
  by the triangle inequality that $$\begin{align*}
          d_{\fu}(\fcc(\fu), q) &\le d_{\fu}(\fcc(\fu), \mfa) + d_{\fu}(\mfa, \fcc(\fu'))\\
          &\quad+ d_{\fu}(\fcc(\fu'), \mfb) + d_{\fu}(\mfb, \fcc(\fu'')) +
          d_{\fu}(\fcc(\fu''), q)\,.
  \end{align*}$$ Using \ref{defdp} and the fact that $\scI(\fu) = \scI(\fu') = \scI(\fu'')$
  this equals $$\begin{align*}
          &\quad d_{\fu}(\fcc(\fu), \mfa) + d_{\fu'}(\mfa, \fcc(\fu'))\\
          &\quad+ d_{\fu'}(\fcc(\fu'), \mfb) + d_{\fu''}(\mfb, \fcc(\fu'')) +
          d_{\fu''}(\fcc(\fu''), q)\\
          &< 100 + 100 + 100 + 100 + 1 < 500\,.
  \end{align*}$$ Since $4\fp \lesssim 500 \fu$, it follows that $d_{\fp}(\fcc(\fp), q) < 4 < 10$. We
  have shown that $B_{\fu''}(\fcc(\fu''), 1) \subset B_{\fp}(\fcc(\fp), 10)$, combining this with
  $\scI(\fu'') = \scI(\fu)$ gives $\fu \sim \fu''$.

  For symmetry suppose that $\fu \sim \fu'$. By Lemma `URel.eq`, it follows that
  $\scI(\fu) = \scI(\fu')$ and that there exists
  $\mfa \in B_{\fu}(\fcc(\fu), 100) \cap B_{\fu'}(\fcc(\fu'), 100)$. Again, for $\fu = \fu'$ symmetry
  is obvious, so suppose that $\fu \ne \fu'$. There exists $\fp \in \mathfrak{T}_1(\fu')$, which then
  satisfies $2\fp\lesssim \fu'$ and $\scI(\fp) \neq \scI(\fu')$. By `smul_mono` and
  \ref{eq-sc3}, it follows that $$\begin{equation}
          \label{eq-rel1}
          10\fp \lesssim 4\fp \lesssim 500 \fu'\,.
  \end{equation}$$ If $q \in B_{\fu}(\fcc(\fu),1)$ then we have from the triangle inequality and the
  fact that $\scI(\fu) = \scI(\fu')$: $$\begin{align*}
          d_{\fu'}(\fcc(\fu'), q) &\le d_{\fu'}(\fcc(\fu'), \mfa) + d_{\fu'}(\mfa, \fcc(\fu)) + d_{\fu'}(\fcc(\fu), q)\\
          &= d_{\fu'}(\fcc(\fu'), \mfa) + d_{\fu}(\mfa, \fcc(\fu)) + d_{\fu}(\fcc(\fu), q)\\
          &< 100 + 100 + 1 < 500\,.
  \end{align*}$$ Combining this with \ref{eq-rel1} and \ref{wiggleorder}, we
  get $$\begin{equation*}
       B_{\fu}(\fcc(\fu), 1) \subset B_{\fp}(\fcc(\fp), 10)\, .
  \end{equation*}$$ Since $2\fp \lesssim \fu'$, we have $\scI(\fp) \subset \scI(\fu') = \scI(\fu)$.
  Thus, $10\fp \lesssim \fu$ which completes the proof of $\fu' \sim \fu$.
  -/)
  (latexEnv := "lemma")]
...
@[blueprint
  "C6 forest"
  (proof := /--
  Let $\fp \in \fC_6(k,n,j)$. By \ref{eq-C4-def} and \ref{defc5}, we have
  $\fp \in \fC_3(k,n,j)$. By \ref{eq-L2-def} and \ref{eq-C3-def}, there exists
  $\fu \in \fU_1(k,n,j)$ with $2\fp \lesssim \fu$ and $\scI(\fp) \ne \scI(\fu)$, that is, with
  $\fp \in \mathfrak{T}_1(\fu)$. Then $\mathfrak{T}_1(\fu)$ is clearly nonempty, so
  $\fu \in \fU_2(k,n,j)$. By the definition of $\fU_3(k,n,j)$, there exists $\fu' \in \fU_3(k,n,j)$
  with $\fu \sim \fu'$. By \ref{definesv}, we have $\fp \in \mathfrak{T}_2(\fu')$.
  -/)
  (latexEnv := "lemma")]
...
/--
For each $\fu\in \fU_3(k,n,j)$, the set $\mathfrak{T}_2(\fu)$ satisfies \ref{forest1}.
...
@[blueprint
  "forest geometry"
  (proof := /--
  Let $\fp \in \mathfrak{T}_2(\fu)$. By \ref{definesv}, there exists $\fu' \sim \fu$ with
  $\fp \in \mathfrak{T}_1(\fu')$. Then we have $2\fp \lesssim \fu'$ and $\scI(\fp) \ne \scI(\fu')$, so
  by \ref{eq-sc3} $4\fp \lesssim 500\fu'$. Further, by `URel.eq`, we have that
  $\scI(\fu') = \scI(\fu)$ and there exists
  $\mfa \in B_{\fu'}(\fcc(\fu'),100) \cap B_{\fu}(\fcc(\fu),100)$. Let
  $\mfb \in B_{\fu}(\fcc(\fu), 1)$. Using the triangle inequality and the fact that
  $\scI(\fu') =\scI(\fu)$, we obtain $$\begin{align*}
          d_{\fu'}(\fcc(\fu'), \mfb) &\le d_{\fu'}(\fcc(\fu'), \mfa) + d_{\fu'}(\fcc(\fu), \mfa) + d_{\fu'}(\fcc(\fu), \mfb)\\
          &= d_{\fu'}(\fcc(\fu'), \mfa) + d_{\fu}(\fcc(\fu), \mfa) + d_{\fu}(\fcc(\fu), \mfb)\\
          &< 100 + 100 + 1 < 500\,.
  \end{align*}$$ Combining this with $4\fp \lesssim 500\fu'$, we obtain
  $$B_{\fu}(\fcc(\fu), 1) \subset B_{\fu'}(\fcc(\fu'), 500) \subset B_{\fp}(\fcc(\fp), 4)\,.$$
  Together with $\scI(\fp) \subset \scI(\fu') = \scI(\fu)$, this gives $4\fp \lesssim \fu$, which is
  \ref{forest1}.
  -/)
  (latexEnv := "lemma")]
...
/--
For each $\fu\in \fU_3(k,n,j)$, the set $\mathfrak{T}_2(\fu)$ satisfies the convexity condition
\ref{forest2}.
...
@[blueprint
  "forest convex"
  (proof := /--
  Let $\fp, \fp'' \in \mathfrak{T}_2(\fu)$ and $\fp' \in \fP$ with $\fp \le \fp' \le \fp''$. By
  \ref{definesv} we have $\fp, \fp'' \in \fC_6(k,n,j) \subset \fC_5(k,n,j)$. By
  `ordConnected_C5`, we have $\fp' \in \fC_5(k,n,j)$. Since $\fp \in \fC_6(k,n,j)$ we have
  $\scI(\fp) \not \subset G'$, so $\scI(\fp') \not \subset G'$ and therefore also
  $\fp' \in \fC_6(k,n,j)$.

  By \ref{definesv} there exists $\fu' \in \fU_2(k,n,j)$ with
  $\fp'' \in \mathfrak{T}_1(\fu')$ and hence $2\fp'' \lesssim \fu'$ and $\scI(\fp'') \ne \scI(\fu')$.
  Together this implies $\scI(\fp'') \subsetneq \scI(\fu')$. With the inclusion
  $\scI(\fp') \subset \scI(\fp'')$ from $\fp' \le \fp''$, it follows that
  $\scI(\fp') \subsetneq \scI(\fu')$ and hence $\scI(\fp') \ne \scI(\fu')$. By \ref{eq-sc1}
  and transitivity of $\lesssim$ we further have $2\fp' \lesssim \fu'$, so
  $\fp' \in \mathfrak{T}_1(\fu')$. It follows that $\fp' \in \mathfrak{T}_2(\fu)$, which shows
  \ref{forest2}.
  -/)
  (latexEnv := "lemma")]
...
/--
For each $\fu,\fu'\in \fU_3(k,n,j)$ with $\fu\neq \fu'$ and each $\fp \in \fT_2(\fu)$ with
$\scI(\fp)\subset \scI(\fu')$ we have $$\begin{equation}
...
@[blueprint
  "forest separation"
  (proof := /--
  By the definition \ref{eq-C2-def} of $\fC_2(k,n,j)$, there exists a tile
  $\fp' \in \fC_1(k,n,j)$ with $\fp' \le \fp$ and $\ps(\fp') \le \ps(\fp)- Z(n+1)$. By
  `Grid.dist_mono` we have
  $$d_{\fp}(\fcc(\fp), \fcc(\fu')) \ge 2^{95a Z(n+1)} d_{\fp'}(\fcc(\fp), \fcc(\fu'))\,.$$ By
  \ref{eq-sc1} we have $2\fp' \lesssim 2\fp$, so by transitivity of $\lesssim$ there exists
  $\mathfrak{v} \sim \fu$ with $2\fp' \lesssim \mathfrak{v}$ and $\scI(\fp') \ne \scI(\mathfrak{v})$.
  Since $\fu, \fu'$ are not equivalent under $\sim$, we have $\mathfrak{v} \not \sim \fu'$, thus
  $10\fp' \not\lesssim \fu'$. This implies that there exists
  $q \in B_{\fu'}(\fcc(\fu'), 1) \setminus B_{\fp'}(\fcc(\fp'), 10)$.

  From $\fp' \le \fp$, $\scI(\fp') \subset \scI(\fp) \subset \scI(\fu')$ and `Grid.dist_mono` it then
  follows that $$\begin{align*}
          ...
          &> -1 + 10 - 1 = 8\,.
  \end{align*}$$ The lemma follows by combining the two displays with the fact that $95 a \ge 1$.
  -/)
  (latexEnv := "lemma")]
...
/--
For each $\fu\in \fU_3(k,n,j)$ and each $\fp \in \mathfrak{T}_2(\fu)$ we have $$\begin{equation}
...
@[blueprint
  ...
  $$\fp \le \fp_{Z(n+1)} \le \dotsb \le \fp_0$$ of distinct tiles in $\fC_3(n,k,j)$. We pick such a
  chain and set $\fq = \fp_0$. Then we have from distinctness of the tiles in the chain that
  $\ps(\fp) \le \ps(\fq) - Z(n+1)$. By \ref{eq-C3-def} there exists
  $\fu'' \in \fU_1(k,n,j)$ with $2\fq \lesssim \fu''$ and $\ps(\fq) < \ps(\fu'')$. Then we have in
  particular by `smul_mono` that $10 \fp \lesssim \fu''$. Let $\fu' \sim \fu$ be such that
  $\fp \in \mathfrak{T}_1(\fu')$. By the definition of $\sim$, it follows that $\fu' \sim \fu''$. By
  transitivity of $\sim$, we have $\fu \sim \fu''$. By `URel.eq`, we have $\ps(\fu'') = \ps(\fu)$,
  hence $\ps(\fq) < \ps(\fu)$ and $\ps(\fp) \le \ps(\fq) - Z(n+1) \le \ps(\fu) - Z(n+1) - 1$.

  Thus, there exists some cube $I \in \mathcal{D}$ with $s(I) = \ps(\fu) - Z(n+1) - 1$ and
  $I \subset \scI(\fu)$ and $\scI(\fp) \subset I$. Since $\fp \in \fC_5(k,n,j)$, we have that
  $I \notin \mathcal{L}(\fu)$, so $B(c(I), 8D^{s(I)}) \subset \scI(\fu)$. By the triangle inequality,
  \ref{defineD} and $a \ge 4$, the same then holds for the subcube $\scI(\fp) \subset I$.
  -/)
  (latexEnv := "lemma")]
...
/--
It holds for $k\le n$ that $$\begin{equation}
...
@[blueprint
  "forest stacking"
  (proof := /--
  Suppose that a point $x$ is contained in more than $(4n + 12)2^n$ cubes $\scI(\fu)$ with
  $\fu \in \fU_3(k,n,j)$. Since $\fU_3(k,n,j) \subset \fC_1(k,n,j)$ for each such $\fu$, there exists
  $\mathfrak{m} \in \mathfrak{M}(k,n)$ such that $100\fu \lesssim \mathfrak{m}$. We fix such an
  $\mathfrak{m}(\fu) := \mathfrak{m}$ for each $\fu$, and claim that the map
  $\fu \mapsto\mathfrak{m}(\fu)$ is injective. Indeed, assume for $\fu\neq \fu'$ there is
  $\mathfrak{m} \in \mathfrak{M}(k,n)$ such that $100\fu \lesssim \mathfrak{m}$ and
  $100\fu' \lesssim \mathfrak{m}$. By \ref{dyadicproperty}, either
  $\scI(\fu) \subset \scI(\fu')$ or $\scI(\fu') \subset \scI(\fu)$. By \ref{defunkj},
  $B_{\fu}(\fcc(\fu),100) \cap B_{\fu'}(\fcc(\fu'), 100) = \emptyset$. This contradicts
  $\Omega(\mathfrak{m})$ being contained in both sets by \ref{eq-freq-comp-ball}.
  Thus $x$ is contained in more than $(4n + 12)2^n$ cubes $\scI(\mathfrak{m})$,
  $\mathfrak{m} \in \mathfrak{M}(k,n)$. Consequently, we have by
  \ref{eq-Aoverlap-def} that $x \in A(2n + 6, k,n) \subset G_2$. Let $\scI(\fu)$ be
  an inclusion minimal cube among the $\scI(\fu'), \fu' \in \fU_3(k,n,j)$ with $x \in \scI(\fu)$. By
  the dyadic property \ref{dyadicproperty}, we have $\scI(\fu) \subset \scI(\fu')$
  for all cubes $\scI(\fu')$ containing $x$. Thus
  $$\scI(\fu) \subset \{y \ : \ \sum_{\fu \in \fU_3(k,n,j)} \mathbf{1}_{\scI(\fu)}(y) > 1 + (4n+12)2^{n}\} \subset G_2\,.$$
  Thus $\mathfrak{T}_1(\fu) \cap \fC_6(k,n,j) = \emptyset$. This contradicts $\fu \in \fU_2(k,n,j)$.
  -/)
  (latexEnv := "lemma")]
...
\end{equation}$$ For all $f:X\to \C$ with $|f|\le \mathbf{1}_F$ we have $$\begin{equation}
        \label{disclesssim1}
...
@[blueprint
  "forest union"
  (proof := /--
  We first fix $k,n, j$. By \ref{definetp} and \ref{defineep}, we have that
  $\mathbf{1}_{\scI(\fp)} T_{\fp}f(x) = T_{\fp}f(x)$ and hence
  $\mathbf{1}_{G \setminus G'} T_{\fp}f(x)= 0$ for all $\fp \in \fC_5(k,n,j) \setminus \fC_6(k,n,j)$.
  Thus it suffices to estimate the contribution of the sets $\fC_6(k,n,j)$. By `forest_stacking`, we
  can decompose $\fU_3(k,n,j)$ as a disjoint union of at most $4n + 12$ collections $\fU_4(k,n,j,l)$,
  $1 \le l \le 4n+12$, each satisfying
  $$\sum_{\fu \in \fU_4(k,n,j,l)} \mathbf{1}_{\scI(\fu)} \le 2^n\,.$$ By Lemmas `forest_geometry`,
  `forest_convex`, `forest_separation`, `forest_inner` and `dens1_le`, the pairs
  $$(\fU_4(k,n,j,l), \mathfrak{T}_2|_{\fU_4(k,n,j,l)})$$ are $n$-forests for each $k,n,j,l$, and by
  `C6_forest`, we have
  ...
  \ref{disclesssim1} and applying `forest_operator` to each term, we obtain the
  estimate
  ...
  for the left hand side of \ref{disclesssim1}. Since $|f| \le \mathbf{1}_F$, we have
  $\|f\|_2 \le \mu(F)^{1/2}$, and we have $\|\mathbf{1}_{G\setminus G'}\|_2 \le \mu(G)^{1/2}$. We get
  a bound $$2^{440a^3} \mu(F)^{\frac{1}{q}} \mu(G)^{1
  ...
  $$2^{2a+5} \sum_{n \ge 0} (2n+3)(4n+12) (n+1) 2^{-\frac{q-1}{q}n}\,.$$ Up to an explicit constant,
  the sum is bounded by $\sum n^3 2^{-\frac{q-1}{q}n}$, which is at most some constant times
  $1/(q-1)^4$ by comparing to an integral. Since $a \ge 4$, this is overall bounded by
  $2^{a^3}/(q-1)^4$, which completes the proof of the lemma.
  -/)
  (latexEnv := "lemma")]
