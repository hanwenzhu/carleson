...
@[blueprint
  "maximal-theorem"
  (title := "Maximal theorem")
  (statement := /-- Let $f: X \to \C$ be bounded, measurable, supported on a set of finite measure,
    and let $\alpha > 0$. Then
    \begin{equation}
        \label{maximal-theorem-equation}
        \mu(\{x\in X : Mf(x) > \alpha\}) \le \frac{2^{2a}}{\alpha} \int |f(y)|\, d\mu(y).
    \end{equation} -/)
  (proof := /-- By definition, for each $x\in X$ with $Mf(x) > \alpha$, there exists a ball $B_x$
    such that $x\in B_x$ and
    \begin{equation}
        \label{maximal-theorem-a}
        \alpha \mu(B_x) < \int_{B_x} |f(y)|\, d\mu(y).
    \end{equation}
    Since $\{x\in X : Mf(x) > \alpha\}$ is open and $\mu$ is inner regular on open sets, it suffices
    to show that
    \begin{equation*}
        \mu(E) \le \frac{2^{2a}}{\alpha} \int |f(y)|\, d\mu(y)
    \end{equation*}
    for every compact $E\subset \{x\in X : Mf(x) > \alpha\}$.
    For such an $E$, by compactness, we can select a finite subcollection $\mathcal{B} \subset
    \{B_x: x\in E\}$ that covers $E$.
...
@[blueprint
  ...
  (title := "Lebesgue differentiation")
  (statement := /-- Let $f$ be a bounded measurable function supported on a set of finite measure.
    Then for $\mu$ almost every $x$, we have
    $$\lim_{n\to \infty} \frac{1}{\mu(B_n)}\int_{B_n} f(y)\, dy= f(x),$$
    where $\{B_n\}_{n\geq 1}$ is a sequence of balls with radii $r_n>0$ such that $x\in B_n$ for
    each $n\geq 1$ and
    \begin{equation*}
        \lim_{n\to \infty} r_n=0 \,.
    \end{equation*} -/)
  (proof := /-- This follows from the Lebesgue differentiation theorem, which is already formalized
    in Lean.
...
attribute [blueprint
  ...
  (title := "Disjoint family countable")
  (statement := /-- In a doubling metric measure space $(X,\rho,\mu, a)$, every disjoint family of
    balls $B_j = B(x_j, r_j)$, $j\in J$, is countable. -/)
  (proof := /-- Choose an arbitrary $x\in X$ as reference point. For $q, Q\in\Q_+$, let $J_{q,Q}$
    denote the set of all $j\in J$ such that $B_j\subset B(x, Q)$ and $r_j \ge q$. It suffices to
    show that all the $J_{q,Q}$ are finite.
    ...
    \begin{equation}
        |J_{q,Q}| \mu(B(x, Q)) \le 2^{a\log_2{\lceil\frac{2Q}{q}\rceil}} \sum_{j\in J_{q,Q}}
        \mu(B_j) \le 2^{a\log_2{\lceil\frac{2Q}{q}\rceil}} \mu(B(x,Q))
    \end{equation}
...
@[blueprint
  "ball-covering"
  (title := "Ball covering")
  (statement := /-- Given an open set $O\ne X$, there exists a countable family of balls $B_j =
    B(x_j, r_j)$ such that
    \begin{equation}
    ...
    \end{equation}
    and we have the bounded intersection property that each $x\in O$ is contained in at most
    $2^{6a}$ of the $B_{3,j}$. -/)
  (proof := /-- Define for $x\in O$,
    \begin{equation}
    ...
    \begin{equation}
        0 < \delta(x) < \infty \,.
    \end{equation}
    Using Zorn's Lemma, we select a maximal disjoint subfamily of $\{B(x,\frac{\delta(x)}{6}) : x
    \in O\}$.
    We obtain a (by \Cref{disjoint-family-countable} countable) family of balls $B_j = B(x_j,
    \frac{\delta(x_j)}{6}), j \in J$ such that \eqref{balls-disjoint},
    \eqref{enlarged-balls-intersect-complement}, and $\bigcup_j B_{3,j} \subset O$ are also
    immediate. For the other inclusion, first observe that for $x,y\in X$,  if
    $B(x,\frac{\delta(x)}{6}) \cap B(y,\frac{\delta(y)}{6}) \ne \emptyset$, then
    \begin{equation*}
        \delta(x) \le \rho(x,y) + \delta(y) \le (\frac{\delta(x)}{6} + \frac{\delta(y)}{6}) +
        \delta(y) = \frac{\delta(x)}{6} + \frac{7\delta(y)}{6},
    \end{equation*}
    so
    \begin{equation}
    ...
    \end{equation}
    Now let $z\in O$. By maximality, there exists some $j\in J$ with $B(z,\frac{\delta(z)}{6}) \cap
    ...
    \begin{equation}
        ...
        \delta(z) \le \rho(z,x_{j_k}) + \delta(x_{j_k}) \le \frac{\delta(x_{j_k})}{2} +
        \delta(x_{j_k}) = \frac{3\delta(x_{j_k})}{2}
    \end{equation}
    and
    \begin{equation*}
    ...
    By \eqref{control-distance-growth-b} and \eqref{control-distance-growth-c}, for all $1\le k \le
    N$, $B(z,\frac{\delta(z)}{6}) \subset B(x_{j_k}, 5r_{j_k})$ and $B_{j_k} \subset
    B(z,\frac{8\delta(z)}{6})$. Using this and \eqref{balls-disjoint}, we obtain
    \begin{align}
        N \mu(B(z,\frac{\delta(z)}{6})) &\le \sum_{k=1}^N \mu(B(x_{j_k}, 5r_j)) \le 2^{3a}
        \sum_{k=1}^N \mu(B_{j_k}) \\
        &= 2^{3a} \mu(\bigcup_{k=1}^N B_{j_k}) \le 2^{3a} \mu(B(z,\frac{8\delta(z)}{6})) \le 2^{6a}
        \mu(B(z,\frac{\delta(z)}{6}))
    \end{align}
...
@[blueprint
  ...
  (statement := /-- Let $f$ be a bounded, a.e. measurable function supported on a set of finite
    measure and let $\alpha>\frac{1}{\mu(X)}\int |f|\,d\mu$.
    Then there exists a bounded a.e. measurable function $g$ supported on a set of finite measure, a
    countable family of balls $B_{3,j}$ (where we allow $B_{3,1} = X$ in the special case that
    $\mu(X)<\infty$)
    % Do we actually need this parenthetical?
    such that each $x\in X$ is contained in at most $2^{6a}$ of the $B_{3,j}$, and a countable
    family of a.e. measurable functions $\{b_j\}_{j\in J}$ such that for all $x \in X$
    \begin{equation}
       \label{eq-gb-dec}
    ...
    \end{equation} -/)
  (proof := /-- Let $E_\alpha:=\{x\in X: Mf(x)>\alpha\}$.
    Then $E_\alpha$ is open. Assume first that $E_\alpha \ne X$. We apply \Cref{ball-covering} with
    $O=E_\alpha$ to obtain the family $B_j, j\in J,$. Without loss of generality, we can assume
    $J=\N$. Define inductively
    \begin{equation}
        Q_j := B_{3,j} \setminus \left(\bigcup_{i<j} Q_i \cup \bigcup_{i>j} B_i \right).
    \end{equation}
    Then $B_j\subset Q_j\subset B_{3,j}$, the $Q_j$ are pairwise disjoint and $\bigcup_j Q_j =
    E_\alpha$.
    Define
    \begin{equation}
        \label{eq-g-def}
    ...
    \end{equation}
    Then \eqref{eq-gb-dec}, \eqref{eq-supp-bj} and \eqref{eq-bad-mean-zero} are true by
    construction.
    Boundedness of $g$ is immediate from the definition, as is the fact that $\supp g$ is contained
    within
    ...
    \end{equation}
    for every ball $B\subset X$ with $x\in B$. It follows by \Cref{Lebesgue-differentiation} that
    for almost every $x\in X\setminus E_\alpha$, $|f(x)|\le \alpha$.
    ...
    \begin{equation}
        |g(x)| \le \frac{1}{\mu(Q_j)}\int_{Q_j} |f(y)|\, d\mu(y) \le
        \frac{1}{\mu(B_j)}\int_{B_{7,j}} |f(y)| \, d\mu(y) \le 2^{3a}\alpha .
    \end{equation}
    
    To prove \eqref{eq-g-L1-norm}, we estimate
    \begin{align*}
        \int |g(z)|\, d\mu(z) &\le \int_{X\setminus E_\alpha} |f(z)|\, d\mu(z) + \sum_{j}
        \int_{Q_j}\frac{1}{\mu(Q_j)}\int_{Q_j}|f(y)|\, d\mu(y)\,d\mu(z) \\
        &= \int |f(z)|\,d\mu(z).
    \end{align*}
    Using the triangle inequality, we have that
    \begin{align}
        \label{eq-bj-int}
        \int_{B_{3,j}} |b_j(y)|\, dy &\le \int_{Q_j} |f(y)|\, d\mu(y) + \int_{Q_j}
        \frac{1}{\mu(Q_j)}\int_{Q_j} |f(x)|\, d\mu(x)\, d\mu(y) \\
        &= 2 \int_{Q_j} |f(y)|\, dy.
    \end{align}
    ...
    \begin{equation}
        \sum_j \mu(B_{3,j}) \le 2^{2a} \sum_j \mu(B_j) \le 2^{2a} \mu(E_\alpha) \le
        \frac{2^{6a}}{\alpha}\int |f(y)|\, d\mu(y),
    \end{equation}
    proving \eqref{eq-bset-length-sum}.
    
    Assume now that $E_\alpha = X$. It follows from \Cref{maximal-theorem} that then
    $\mu(X)<\infty$.
    Define
    \begin{equation*}
    ...
    \begin{equation*}
        b_1 := f - g.
    \end{equation*}
    Then $f = g + b_1$ and $\supp b_1 \subset B_{3,1}:=X$ and \eqref{eq-gb-dec},
    \eqref{eq-g-L1-norm}, \eqref{eq-supp-bj}, \eqref{eq-bad-mean-zero} all hold immediately. By
    assumption, $\alpha>\frac{1}{\mu(X)}\int |f|\,d\mu = g$, so \eqref{eq-g-max} holds.
...
@[blueprint
  ...
  (proof := /-- We estimate using monotonicity of the integral
    \begin{equation*}
         \mu\left(\{x\in X: |T_r g(x)|>{\alpha}/2\}\right)\leq \frac{4}{\alpha^2} \int |T_r
         g(y)|^2\, d\mu(y).
    \end{equation*}
    Using \eqref{eq-strong-2-2-assumption} followed by \eqref{eq-g-max} and \eqref{eq-g-L1-norm}, we
    estimate the right hand side above by
    \begin{equation}
        \label{eq-Hr-g}
        \leq \frac{4\cdot 2^{2a^3}}{\alpha^2} \int |g(y)|^2\, d\mu(y)\leq
        \frac{2^{2a^3+3a+2}c}{\alpha} \int |g(y)|\, dy \le \frac{2^{2a^3+3a+2}c}{\alpha} \int
        |f(y)|\, d\mu(y).
    \end{equation} -/)
  (latexEnv := "lemma")]
...
private lemma 𝒥₂_bound (hf : BoundedFiniteSupport f) (hα : 0 < α) (hx : x ∈ (Ω f (α' a α))ᶜ)
  ...
  _ ≤ _ := by
    ...
    · apply le_trans (enorm_integral_le_lintegral_enorm _)
      ...
      exact mul_le_mul_right' (enorm_d_le hf hX j) _
...
private lemma tsum_𝒥₂ (hf : BoundedFiniteSupport f) (hα : 0 < α) (hx : x ∈ (Ω f (α' a α))ᶜ)
  ...
  _ ≤ _ := by
    ...
    · calc
        ...
        _ ≤ _ * 2 ^ (a^3 + 10*a) := mul_le_mul_left' (tsum_integral_K_le hx hX) _
...
@[blueprint
  ...
  (title := "Estimate bad partial")
  (statement := /-- Let $x\in X\setminus\Omega$. Then
    \begin{equation*}
    ...
    \begin{equation*}
        ...
        \left(\frac{3r_j}{\rho(x,x_j)}\right)^{\frac{1}{a}}\frac{\mu(B_{3,j})}{V(x,x_j)}.
    \end{equation*} -/)
  (proof := /-- We decompose the index set $J$ into the following disjoint sets:
    \begin{align*}
    ...
    \end{alignat}
    For all $j\in \mathcal{J}_3(x)$, $\supp b_j\subset B_{3,j}\subset B(x,r)$, and thus
    $T_rb_j(x)=0$, so $\eqref{eq-b-dec-3} = 0$.
    
    Next, for $j\in \mathcal{J}_1(x)$, $\supp b_j\subset B_{3,j}\subset X \setminus B(x,r)$, and we
    have
    \begin{equation*}
        T_rb_j(x)=\int_{X\setminus B(x,r)} K(x,y) b_j(y)\,d\mu(y)=\int_{B_{3,j}} K(x,y)
        b_j(y)\,d\mu(y)\,.
    \end{equation*}
    ...
    \begin{equation*}
        \eqref{eq-b-dec-1} \le \sum_{j\in
        \mathcal{J}_1(x)}\int_{B_{3,j}}\left(\frac{\rho(x_j,y)}{\rho(x,x_j)}\right)^{\frac{1}{a}}\frac{2^{a^3}}{V(x,x_j)}
        |b_j(y)|\, d\mu(y)
    \end{equation*}
    \begin{equation}
        \le 2^{a^3} \sum_{j}
        \left(\frac{3r_j}{\rho(x,x_j)}\right)^{\frac{1}{a}}\frac{1}{V(x,x_j)}\int_{B_{3,j}}
        |b_j(y)|\, dy
    \end{equation}
    and by \eqref{eq-bj-L1},
    \begin{equation}
        \label{eq-J1-diff-est}
        \le 2^{a^3+2a+1} c\alpha \sum_{j}
        \left(\frac{3r_j}{\rho(x,x_j)}\right)^{\frac{1}{a}}\frac{\mu(B_{3,j})}{V(x,x_j)} = F(x).
    \end{equation}
    ...
    \begin{align*}
        T_r b_j(x)&=\int_{B_{3,j}} K(x,y) (\mathbf{1}_{X\setminus B(x,r)}(y)b_j(y)-d_j)\, dy +
        \int_{B_{3,j}} d_j K(x,y) \, dy \\
        &= \int_{B_{3,j}} (K(x,y)-K(x,x_j)) (\mathbf{1}_{X\setminus B(x,r)}(y)b_j(y)-d_j)\, dy +
        \int_{B_{3,j}} d_j K(x,y) \, dy.
    \end{align*}
    ...
    \begin{equation}
        ...
        \int_{B_{3,j}} |K(x,y)-K(x,x_j)| \left(|b_j(y)|+2^{2a+1} c\alpha\right)\, dy +2^{2a+1}
        c\alpha \int_{B_{3,j}}  |K(x,y)| \, dy.
    \end{equation}
    By \eqref{eq-Om-cj}, we can apply \eqref{eqkernel-y-smooth} and arguing as in
    \eqref{eq-J1-diff-est}, we get that
    \begin{equation}
        \label{eq-J2-diff-est-2}
        \eqref{eq-b-dec-2} \le 2F(x) + 2^{2a+1}c\alpha \sum_{j\in\mathcal{J}_2(x)} \int_{B_{3,j}} 
        |K(x,y)|\,d\mu(y),
    \end{equation}
    ...
    \begin{equation*}
        \rho(x,x_j) \ge r-3r_j \ge r - \frac{1}{2} \rho(x,x_j) \implies \rho(x,x_j) \ge
        \frac{2}{3}r,
    \end{equation*}
    and conclude
    \begin{equation*}
        \rho(x,y) \ge \rho(x,x_j) - \rho(y,x_j) \ge \rho(x,x_j) - 3r_j \ge \rho(x,x_j) -
        \frac{1}{2}\rho(x,x_j) \ge \frac{1}{3} r.
    \end{equation*}
    
    Using the bounded intersection property of the $B_{3,j}$, \eqref{eq-J2-union-subset} and
    \eqref{eqkernel-size}, we get
    \begin{align}
        \sum_{j\in\mathcal{J}_2(x)} \int_{B_{3,j}}  |K(x,y)|\,d\mu(y) &\le 2^{6a} \int_{A} 
        |K(x,y)|\,d\mu(y) \\
        &\le 2^{6a} \int_{B(x,3r) \setminus B(x,\frac{r}{3})}  |K(x,y)|\,d\mu(y) \\
        &\le 2^{6a} \int_{B(x,3r) \setminus B(x,\frac{r}{3})} \frac{2^{a^3}}{V(x,y)} \,d\mu(y) \\
        &\le 2^{a^3+6a} \int_{B(x,3r) \setminus B(x,\frac{r}{3})} \frac{1}{\mu(B(x,\frac{r}{3}))}
        \,d\mu(y) \\
    ...
    Combining the estimates \eqref{eq-J1-diff-est} for \eqref{eq-b-dec-1}, \eqref{eq-J2-diff-est-2}
    for \eqref{eq-b-dec-2}, and \eqref{eq-J2-diff-est-4}, we get
    \begin{equation*}
...
@[blueprint
  "estimate-F-set"
  (title := "Estimate F set")
  (statement := /-- For $F$ as defined in \Cref{estimate-bad-partial}, we have
    \begin{equation}
        \label{eq-F-X-minus-Omega}
        \mu(\{x\in X\setminus\Omega: F(x)>\alpha/8\}) \le \frac{2^{a^3+11a+4}}{\alpha} \int
        |f(y)|\,d\mu(y)\,.
    \end{equation} -/)
  (proof := /-- We estimate
    \begin{align}
        ...
        &\le \frac{8}{\alpha} \int_{X\setminus \Omega} 2^{a^3+2a+1} c\alpha \sum_{j}
        \left(\frac{3r_j}{\rho(x,x_j)}\right)^{\frac{1}{a}}\frac{\mu(B_{3,j})}{V(x,x_j)}\,d\mu(x) \\
        \label{eq-F-est-1}
        &\le 2^{a^3+2a+4} c \sum_{j} \mu(B_{3,j}) \int_{X\setminus B_{6,j}}
        \left(\frac{3r_j}{\rho(x,x_j)}\right)^{\frac{1}{a}}\frac{1}{V(x,x_j)}\,d\mu(x)
    \end{align}
    Using
    \begin{equation*}
        V(x,x_j) = \mu(B(x,\rho(x,x_j))) \ge 2^{-a}\mu(B(x,2\rho(x,x_j))) \ge 2^{-a}
        \mu(B(x_j,\rho(x_j,x))),
    \end{equation*}
    ...
    \begin{align*}
        ...
        \left(\frac{3r_j}{\rho(x,x_j)}\right)^{\frac{1}{a}}\frac{1}{V(x,x_j)}\,d\mu(x) \\
        \le& 2^a \int_{X\setminus B_{6,j}}
        \left(\frac{3r_j}{\rho(x,x_j)}\right)^{\frac{1}{a}}\frac{1}{\mu(B(x_j,\rho(x_j,x)))}\,d\mu(x)
        \\
        \le&2^a \sum_{n=1}^\infty \int_{B(x_j,2^{n+1}\cdot3r_j)\setminus B(x_j,2^n\cdot3r_j)}
        \left(\frac{3r_j}{2^n \cdot3r_j}\right)^{\frac{1}{a}}\frac{1}{\mu(B(x_j,2^n
        \cdot3r_j))}\,d\mu(x) \\
        \le&2^a \sum_{n=1}^\infty 2^{-\frac{n}{a}} \frac{\mu(B(x_j,2^{n+1}\cdot3r_j))}{\mu(B(x_j,2^n
        \cdot3r_j))} \\
        \le&2^{3a},
    \end{align*}
...
@[blueprint
  ...
  (statement := /-- We have
    \begin{equation*}
        \mu\left({\{x\in X: |T_r b(x)|>\alpha/2\}}\right) \le  \frac{\frac{2^{7a}}{c} +
        2^{a^3+11a+4}}{\alpha} \int |f(y)|\,d\mu(y) \,.
    \end{equation*} -/)
  (proof := /-- We estimate
    \begin{equation*}
    ...
    \begin{equation}
        \label{eq-set-dec-3}
        \mu({\{x\in X\setminus\Omega: |T_r b(x)|>\alpha/2\}}) \le \mu(\{x\in X\setminus\Omega: F(x)>
        \alpha/8\})\,.
    \end{equation}
...
@[blueprint
  ...
  (title := "Calderon-Zygmund Weak (1, 1)")
  (statement := /-- Let $f:X\to\C$ be a bounded measurable function supported on a set of finite
    measure and assume for some $r>0$ that for every bounded measurable function $g:X\to\C$
    ...
    \begin{equation}
        \label{eq-weak-1-1}
        \mu\left(\{x\in X: |T_r f(x)|>\alpha\}\right)\le \frac{2^{a^3 + 21a}}{\alpha} \int |f(y)|\,
        d\mu(y).
    \end{equation} -/)
  (proof := /-- [Proof of \Cref{calderon-zygmund-weak-1-1}]
    ...
    \begin{equation*}
    \label{eq-set-dec-1}
     \le \mu\left(\{x\in X: |T_r g(x)|>{\alpha}/2\}\right) +  \mu\left(\{x\in X: |T_r
     b(x)|>{\alpha}/2\}\right).
    \end{equation*}
    Using \Cref{estimate-good}, \Cref{estimate-bad} and the definition
    \eqref{weak-1-1-proof-cz-const} of $c$, we get
    \begin{align*}
