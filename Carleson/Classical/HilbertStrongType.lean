...
open AddCircle in
/--
Let $f$ be a bounded $2\pi$-periodic measurable function. Then, for all $N\ge 0$ $$\begin{equation}
\label{snbound}
...
@[blueprint
  ...
  (proof := /--
  The functions $e_n:x\mapsto e^{inx}$ form an orthonormal basis in $L^2[-\pi, \pi]$ (this is already
  in Mathlib). Therefore we have $$\begin{align*}
      \|S_Nf\|^2_{L^2[-\pi, \pi]}
...
/--
We have for every bounded measurable $2\pi$-periodic function $g$ $$\begin{equation}
\label{lnbound}
...
@[blueprint
  ...
  \end{equation}$$ We have by the triangle inequality, the square root of the identity in
  \ref{mnbound}, and `spectral_projection_bound` $$\begin{equation*}
  ...
  \end{equation}$$ This proves \ref{lnbound} and completes the proof of the lemma.
  -/)
  (latexEnv := "lemma")]
...
attribute [blueprint
  ...
  -/)
  (proof := /--
  We have by periodicity and change of variables $$\begin{equation}
  \label{eqhil9}
   \int_{-x}^{0} f(y)\, dy=\int_{-x}^{0} f(y+2\pi)\, dy= \int_{2\pi -x}^{2\pi} f(y)\, dy\, .
  \end{equation}$$ We then have by breaking up the domain of integration and using
  \ref{eqhil9} $$\begin{equation*}
  ...
  \end{equation}$$ This proves the first identity of the lemma. The second identity follows by
  substitution of $y$ by $y-x$.
  -/)
  (latexEnv := "lemma")] Function.Periodic.intervalIntegral_add_eq
...
@[blueprint
  "Young convolution"
  (proof := /--
  Using Fubini and `Function.Periodic.intervalIntegral_add_eq`, we observe $$\begin{equation*}
  ...
  \end{equation}$$ Let $h$ be the nonnegative square root of $g$, then $h$ is bounded and
  $2\pi$-periodic with $h^2=g$. We estimate the square of the left-hand side of
  \ref{eqyoung} with Cauchy-Schwarz and then with \ref{eqhil4} by
  $$\begin{equation*}
  ...
  \end{equation*}$$ Taking square roots, this proves the lemma.
  -/)
  (latexEnv := "lemma")]
...
/--
Let $g,f$ be bounded measurable $2\pi$-periodic functions. Let $0<r<\pi$. Assume we have for all $x$
$$\begin{equation}
\label{ebump1}
    |g(x)|\le k_r(x)\, .
...
@[blueprint
  ...
  (proof := /--
  From monotonicity of the integral and \ref{ebump1}, $$\begin{equation}
      \|g\|_{L^1[0, 2\pi]} \le \int_0^{2\pi}k_r(x)\, dx\,.
  \end{equation}$$ Using the symmetry $k_r(x)=k_r(-x)$, the assumption, and `lower_secant_bound'`, the
  last display is equal to $$\begin{equation*}
  ...
  \end{equation*}$$ $$\begin{equation}
      ...
      \le 17\, .
  \end{equation}$$ Together with `young_convolution`, this proves the lemma.
  -/)
  (latexEnv := "lemma")]
...
/--
Let $0<r<1$. Let $N$ be the smallest integer larger than $\frac 1r$. There is a $2\pi$-periodic
continuous function ${L'}$ on $\R$ that satisfies for all $0\le x\le 2\pi$ and all $2\pi$-periodic
bounded measurable functions $f$ on $\R$ $$\begin{equation}
\label{lthroughlprime}
    L_Nf(x)=\frac 1{2\pi}\int_{0}^{2\pi}f(y) {L'}(x-y)\, dy\,.
\end{equation}$$ Moreover, for all $-\pi \le x \le \pi$, $$\begin{equation}
\label{eqdifflhil}
...
@[fun_prop, blueprint
  ...
  (proof := /--
  We have by definition and `dirichletKernel_eq` $$\begin{equation}
      L_Ng(x)=
      \frac 1N\sum_{n=0}^{N-1}
         \int_0^{2\pi} e^{-i(N+n)x} K_{N+n}(x-y) e^{i(N+n)y}g(y)
  \, dy \, .
  \end{equation}$$ This is of the form \ref{lthroughlprime} with the continuous
  function $$\begin{equation}
      {L'}(x)= \frac 1N\sum_{n=0}^{N-1}
        K_{N+n}(x) e^{-i(N+n)x}\, .
  \end{equation}$$ With \ref{eqksumexp} of `dirichletKernel_eq` we have $|K_N(x)|\le 2N+1$
  for every $x$ and thus $$\begin{equation}
  \label{eqhil13}
      |{L'}(x)|\le \frac 1N\sum_{n=0}^{N-1}
        (2N+2n+1) \le 4N\le 2^3 r^{-1}\, .
  \end{equation}$$ Therefore, for $|x|\in [0, r)\cup (1, \pi]$, we have $$\begin{equation}
      \label{eqdiffzero}
      \left|L'(x)-\mathbf{1}_{\{y:\, r<|y|<1\}}(x)\kappa(x)\right|=|L'(x)|\leq 2^{3} r^{-1}.
  \end{equation}$$ This proves \ref{eqdifflhil} for $|x|\in [0, r)$ since $k_r(x)=r^{-1}$
  in this case.
  
  For $e^{ix}\neq 1$ one may use the expression \ref{eqksumhil} for $K_N$ in
  `dirichletKernel_eq` to obtain $$\begin{equation*}
      {L'}(x)= \frac 1N\sum_{n=0}^{N-1}
  ...
  \end{equation}$$ where $$L''(x):=\frac 1N \frac {e^{i2Nx}}{1-e^{-ix}}
       \sum_{n=0}^{N-1}
      {e^{i2nx}}.$$ For $x\in [-\pi, r]\cup [r, \pi]$, we have using `lower_secant_bound'` that
  $$\begin{equation*}
  ...
  \end{equation}$$ Next, we need to estimate $L''(x)$. If the real part of $e^{ix}$ is negative, we
  have $$\begin{equation}
    1\le |1-e^{-ix}|\le 2\, .
  ...
  \end{equation}$$ If the real part of $e^{ix}$ is positive and in particular while still
  $e^{ix}\neq \pm 1$, then we have by telescoping $$\begin{equation}
   (1-e^{2ix})
       \sum_{n=0}^{N-1}
      {e^{i2nx}}=1-e^{i2Nx}\, .
  \end{equation}$$ As $e^{2ix}\neq 1$, we may divide by $1-e^{2ix}$ and insert this into
  \ref{eqhil3} to obtain $$\begin{equation}
   L''(x)=
             \frac 1N \frac {e^{i2Nx}}{1-e^{-ix}}
       \frac{1-e^{i2Nx}}{1-e^{2ix}}\, .
  \end{equation}$$ Hence, using the nonnegativity of the real part of $e^{ix}$ $$\begin{equation*}
      |L''(x)|
  ...
  \end{equation}$$ Moreover, for $|x| \in [r, \pi]$, one checks easily that $$\begin{equation*}
    1+\frac {r}{|1-e^{ix}|^2} \leq 5 k_r(x).
  \end{equation*}$$ Therefore, \ref{eqhil12} and \ref{eqhil11} show that, in this
  range of $x$, $$\begin{equation*}
    |L''(x)| \le 10 k_r(x).
  \end{equation*}$$ Together with \ref{eq-diffzero2}, this prove
  \ref{eqdifflhil} for $|x| \in [r, \pi]$. As the range $|x| \in [0,r)$ is covered by
  \ref{eqdiffzero}, this completes the proof of the lemma.
  -/)
  (latexEnv := "lemma")]
...
-/
/--
Let $0<r$. Let $f$ be a bounded, measurable function on $\mathbb{R}$. Then $$\begin{equation}
...
@[blueprint
  "Hilbert strong 2 2"
  (proof := /--
  We first show that if $f$ is supported in $[1, 4]$, then $$\begin{equation}
          \label{eq-Hr-short-support}
          \|H_r f\|_{L^2[2, 3]} \le 2^{8} \|f\|_{L^2(\R)}\,.
  \end{equation}$$ Let $\tilde{f}$ be the $2\pi$-periodic extension of $f$ to $\mathbb{R}$. Let $N$ be
  the smallest integer larger than $\frac 1r$. Then, \ref{eqdifflhil} shows that the
  kernels of $H_r$ and $2\pi L_N$ differ by at most $12k_r$ on $[-\pi, \pi]$. Consider $x \in [2,3]$.
  When computing $H_r f(x)$ and $2\pi L_N f(x)$, the kernels are computed at points of the form $x-y$
  with $f(y) \ne 0$, i.e., $y \in [1,4]$. As $x\in [2,3]$, the difference $x-y$ is bounded in absolute
  value by $2$, and therefore belongs to $[-\pi, \pi]$, where the above bound holds.
  
  It follows that, for $x \in [2,3]$, $$\begin{equation*}
          |H_r \tilde{f}(x)|\leq 2\pi |L_N \tilde{f}(x)|+12\left|\int_{0}^{2\pi}k_r(x-y)\tilde{f}(y)\, dy\right|.
  \end{equation*}$$ Taking $L^2$ norm and using its sub-additivity, we get $$\begin{equation*}
  ...
  \end{equation*}$$ Since $\kappa$ is supported in $[-1,1]$, we have that $H_r\tilde{f}$ agrees with
  $H_r f$ on $[2,3]$. Using `modulated_averaged_projection` and `integrable_bump_convolution`, we
  conclude $$\begin{equation}
          \|H_r f\|_{L^2[2, 3]} \le 2\pi \|f\|_{L^2[0, 2\pi]} + 12 \cdot 17 \|f\|_{L^2[0, 2\pi]}\,,
  \end{equation}$$ which gives \ref{eq-Hr-short-support}.
  
  Suppose now that $f$ is supported in $[c, c+3]$ for some $c \in \R$. Then the function
  $g(x) = f(x-c+1)$ is supported in $[1, 4]$. By a change of variables in \ref{def-H-r}, we
  have $H_r g(x ) = H_r f(x - c + 1)$. Thus, by \ref{eq-Hr-short-support}
  $$\begin{equation}
          \label{eq-Hr-short-support-2}
          \|H_rf\|_{L^2[c+1, c+2]} = \|H_r g\|_{L^2[2,3]} \le 2^{8} \|g\|_2 = 2^8 \|f\|_2\,.
  \end{equation}$$
  
  Let now $f$ be arbitrary. Since $\kappa(x) = 0$ for $|x| > 1$, we have for all $x \in [c+1, c+2]$
  $$H_rf(x) = H_r(f \mathbf{1}_{[c, c+3]})(x)\,.$$ Thus
  $$\int_{c+1}^{c+2} |H_r f(x)|^2 \, \mathrm{d}x = \int_{c+1}^{c+2}|H_r(f \mathbf{1}_{[c, c+3]})(x)|^2 \, \mathrm{d}x\,.$$
  Applying the bound \ref{eq-Hr-short-support-2}, this is
  $$\le 2^{16} \int_{c}^{c+3} |f(x)|^2 \, \mathrm{d}x\,.$$ Summing over all $c \in \mathbb{Z}$, we
  obtain
