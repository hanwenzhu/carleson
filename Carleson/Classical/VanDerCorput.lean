...
\end{equation}$$ Then for any $\alpha \le \beta$ and $n\in\Z$ we have $$\begin{equation}
...
@[blueprint
  ...
  \end{equation*}$$ Assume now $n\ne 0$. Without loss of generality, we may assume $n>0$. We
  distinguish two cases. If $\beta-\alpha < \frac{\pi}{n}$, we have by the triangle inequality
  $$\begin{equation*}
  ...
  \end{equation*}$$ We turn to the case $\frac{\pi}{n} \le \beta-\alpha$. We have
  $$e^{in(x + \pi/n)} = -e^{inx}\,.$$ Using this, we write
  ...
  We split the first integral at $\alpha + \frac{\pi}{n}$ and the second one at
  $\beta - \frac{\pi}{n}$, and make a change of variables in the second part of the first integral to
  obtain
  ...
  $$\frac{\pi}{n} \sup_{x \in [\alpha,\beta]} |g(x)|\,.$$ The third term is by the triangle inequality
  at most
