...
/--
Let $f$ be a $2\pi$-periodic complex-valued continuous function on $\mathbb{R}$. For all
$\epsilon>0$, there exists a Borel set $E\subset [0,2\pi]$ with Lebesgue measure $|E|\le \epsilon$
and a positive integer $N_0$ such that for all $x\in [0,2\pi]\setminus E$ and all integers $N>N_0$,
...
@[blueprint
  ...
  (proof := /--
  Let $N_0$ be as in `fourierConv_ofTwiceDifferentiable`. For every $$\begin{equation}
  x\in [0, 2\pi) \setminus E\, ,
  \end{equation}$$ and every $N>N_0$ we have by the triangle inequality $$\begin{equation*}
      |f(x)-S_Nf(x)|
  \end{equation*}$$ $$\begin{equation}
  \label{epsilonthird}
      \le |f(x)-f_0(x)|+ |f_0(x)-S_Nf_0(x)|+|S_Nf_0(x)-S_N f(x)|\, .
  \end{equation}$$ Using `close_smooth_approx_periodic`, `fourierConv_ofTwiceDifferentiable`,
  `control_approximation_effect`, we estimate \ref{epsilonthird} by $$\begin{equation}
      \le \epsilon' +\frac \epsilon 4 +\frac \epsilon 4\le \epsilon\, .
  \end{equation}$$ This shows \ref{aeconv} for the given $E$ and $N_0$.
  -/)]
...
/--
Let $f$ be a $2\pi$-periodic complex-valued continuous function on $\mathbb{R}$. Then for almost all
$x \in \mathbb{R}$ we have $$\begin{equation}
\label{eq:fourier-limit}
      \lim_{N\to\infty}S_N f(x) = f(x),
\end{equation}$$ where $S_N f$ is the $N$-th partial Fourier sum of $f$ defined in
\ref{eq:trig-series} with coefficients
\ref{eq:fourier-coefficients}.
-/
@[blueprint
  "classical Carleson"
  (proof := /--
  By applying `exceptional_set_carleson` with a sequence of $\epsilon_n:= 2^{-n}\delta$ for $n\ge 1$
  and taking the union of corresponding exceptional sets $E_n$, we see that outside a set of measure
  $\delta$, the partial Fourier sums converge pointwise for $N\to \infty$. Applying this with a
  sequence of $\delta$ shrinking to zero and taking the intersection of the corresponding exceptional
  sets, which has measure zero, we see that the Fourier series converges outside a set of measure
  zero.
  -/)]
