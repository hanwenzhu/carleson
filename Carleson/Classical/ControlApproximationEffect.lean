...
/--
For all $N\in\Z$ and $x\in [-\pi,\pi] \setminus \{0\}$, $$\begin{equation*}
...
@[blueprint
  ...
  (proof := /--
  Let $N\in\Z$ and $x\in [-\pi,\pi] \setminus \{0\}$. With `dirichletKernel_eq`, we obtain
  $$\begin{equation*}
          K_N(x) - (e^{-iNx}\kappa(x) + \overline{e^{-iNx}\kappa(x)})
          = e^{-iNx} \frac{\min(|x|, 1) }{1 - e^{ix}} + e^{iNx} \frac{\min(|x|, 1) }{1 - e^{-ix}} \,.
  \end{equation*}$$ Using `lower_secant_bound'` with $\eta = \min(|x|, 1)$, we bound
  $$\begin{equation*}
...
/--
Let $g:\R\to\C$ be a measurable $2\pi$-periodic function such that for some $\delta>0$ and every
$x\in\R$, $$\begin{equation}
        |g(x)|\le \delta \,.
\end{equation}$$ Then for every $x\in [0,2\pi]$ and $N>0$, $$\begin{equation*}
...
@[blueprint
  ...
  (proof := /--
  Let $x\in [0,2\pi]$ and $N>0$. We have with `dirichletKernel_eq` $$\begin{equation*}
          |S_N g(x)| = \frac{1}{2\pi} \left| \int_0^{2\pi} g(y) K_N(x-y) \, dy\right|\,.
  \end{equation*}$$ We use $2\pi$-periodicity of $g$ and $K_N$ to shift the domain of integration to
  obtain $$\begin{equation*}
          = \frac{1}{2\pi} \left|\int_{x-\pi}^{x+\pi} g(y) K_N(x-y) \, dy\right|\,.
  \end{equation*}$$ Using the triangle inequality, we split this as $$\begin{equation}
  ...
  \end{equation}$$ Note that all integrals are well defined, since $K_N$ is by
  \ref{eqksumexp} bounded by $2N+1$. Using that $$\begin{equation}
          \label{eq-Dirichlet-Hilbert}
          \max(|x-y|,0) K_N(x-y) = e^{-iN(x-y)}\kappa(x-y) + \overline{e^{-iN(x-y)}\kappa(x-y)} \,,
  \end{equation}$$ `Dirichlet_Hilbert_diff` and \ref{g-small}, we bound
  \ref{eq-diff-integrable} by $$\begin{equation*}
          \frac{1}{2\pi} \int_{x-\pi}^{x+\pi} |g(y)| \left|K_N(x-y) - e^{-iN(x-y)}\kappa(x-y) + \overline{e^{-iN(x-y)}\kappa(x-y)}\right|\, dy
          \le \pi\delta \,.
  \end{equation*}$$ By dominated convergence and since $\kappa(x-y) = 0$ for $|x-y| > 1$,
  \ref{eq-diff-singular} equals $$\begin{equation*}
          \frac{1}{2\pi} \lim_{r \to 0^+} \left| \int_{r < |x-y| < 1} g(y) \max(|x-y|,0) K_N(x-y) \, dy\right|\,.
  \end{equation*}$$ We bound the limit by a supremum and rewrite using
  \ref{eq-Dirichlet-Hilbert}, $$\begin{equation*}
          \le \frac{1}{2\pi} \sup_{r > 0} \left| \int_{r < |x-y| < 1} g(y) \left(e^{-iN(x-y)}\kappa(x-y) + \overline{e^{-iN(x-y)}\kappa(x-y)}\right) \, dy\right|
  \end{equation*}$$ Using the triangle inequality, we further bound this by $$\begin{alignat*}
  {3}
  ...
  \end{alignat*}$$ By the definition \ref{define-T-carleson} of $T$, this is
  $$\begin{equation*}
...
/--
There is a set $E \subset \R$ with Lebesgue measure $|E|\le \epsilon$ such that for all
$$\begin{equation}
...
@[blueprint
  ...
  (proof := /--
  `control_approximation_effect` follows directly from `control_approximation_effect` with
  $\delta:=\epsilon'$.
  -/)
  (latexEnv := "lemma")]
