...
@[blueprint
  ...
  (proof := /--
  Fix $x\neq y$. If $\kappa(x-y)$ is zero, then \ref{eqcarl30} is evident. Assume
  $\kappa(x-y)$ is not zero, then $0<|x-y|<1$. We have $$\begin{equation}
  \label{eqcarl31}
  |\kappa(x-y)|=\left|\frac {1-|x-y|}{1-e^{i(x-y)}}\right|\, .
  \end{equation}$$ We estimate with `lower_secant_bound'` $$\begin{equation}
  \label{eqcarl311}
  |\kappa(x-y)|\le \frac {1}{|1-e^{i(x-y)}|}\le \frac 2{|x-y|}\, .
  \end{equation}$$ This proves \ref{eqcarl30} in the given case and completes the proof of
  the lemma.
  -/)
  (latexEnv := "lemma")]
...
@[blueprint
  ...
  (proof := /--
  Upon replacing $y$ by $y-x$ and $y'$ by $y'-x$ on the left-hand side of
  \ref{eq-close-hoelder}, we can assume that $x = 0$. Then the assumption
  \ref{eq-close-hoelder} implies that $y$ and $y'$ have the same sign. Since
  $\kappa(y) = \bar \kappa(-y)$ we can assume that they are both positive. Then it follows from
  \ref{eq-close-hoelder} that $$\frac{y}{2} \le y' \,.$$ We distinguish four cases.
  If $y, y' \le 1$, then we have
  $$|\kappa(-y) - \kappa(-y')| = \left| \frac{1 - y}{1- e^{-iy}} - \frac{1 - y'}{1- e^{-iy'}}\right|$$
  and by the fundamental theorem of calculus
  $$= \left| \int_{y'}^{y} \frac{-1 + e^{-it} + i(1-t)e^{it}}{(1 - e^{-it})^2} \,dt \right|\,.$$ Using
  $y' \ge \frac{y}{2}$ and `lower_secant_bound'`, we bound this by
  ...
  $$\le 2^{6} \frac{|y-y'|}{|y / 2|^2} = 2^{8} \frac{|y-y'|}{|y|^2}$$ Finally, if $y, y' > 1$ then
