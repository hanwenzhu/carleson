...
@[blueprint
  ...
  (title := "local dens1 tree bound")
  (statement := /-- Let $\fu \in \fU$ and $L \in \mathcal{L}(\fT(\fu))$. Then
    \begin{equation}
    \label{eq-1density-estimate-tree}
        \mu(L \cap G \cap \bigcup_{\fp \in \fT(\fu)} E(\fp)) \le 2^{101a^3} \dens_1(\fT(\fu))
        \mu(L)\,.
    \end{equation} -/)
  (proof := /-- [Proof of \Cref{local-dens1-tree-bound}]
    
    
        If the set on the right hand side is empty, then \eqref{eq-1density-estimate-tree} holds. If
        not, then there exists $\fp \in \fT(\fu)$ with $L \cap \scI(\fp) \ne \emptyset$.
    
        Suppose first that there exists such $\fp$ with $\ps(\fp) \le s(L)$. Then by
        \eqref{dyadicproperty} $\scI(\fp) \subset L$, which gives by the definition of
        $\mathcal{L}(\fT(\fu))$ that $s(L) = -S$ and hence $L = \scI(\fp)$. Let $\fq \in \fT(\fu)$
        with $E(\fq) \cap L \ne \emptyset$. Since $s(L) = -S \le \ps(\fq)$ it follows from
        \eqref{dyadicproperty} that $\scI(\fp) = L \subset \scI(\fq)$. We have then by
        \Cref{monotone-cube-metrics}
        \begin{align*}
            d_{\fp}(\fcc(\fp), \fcc(\fq)) &\le d_{\fp}(\fcc(\fp), \fcc(\fu)) + d_{\fp}(\fcc(\fq),
            \fcc(\fu))\\
            &\le d_{\fp}(\fcc(\fp), \fcc(\fu)) + d_{\fq}(\fcc(\fq), \fcc(\fu))\,.
        \end{align*}
        Using that $\fp, \fq \in \fT(\fu)$ and \eqref{forest1}, this is at most $8$. Using again the
        triangle inequality and \Cref{monotone-cube-metrics}, we obtain that for each $q \in
        B_{\fq}(\fcc(\fq), 1)$
        $$
        ...
        $\ps(\fp) > s(L)$.
        Since there exists at least one such $\fp$, there exists in particular at least one cube
        $L'' \in \mathcal{D}$ with $L \subset L''$ and $s(L'') > s(L)$.
        By \eqref{coverdyadic}, there exists $L' \in \mathcal{D}$ with $L \subset L'$ and $s(L') =
        s(L) + 1$.
        By the definition of $\mathcal{L}(\fT(\fu))$ there exists a tile $\fp'' \in \fT(\fu)$ with
        $\scI(\fp'') \subset L'$.
    
        It suffices to show that there exists a tile $\fp' \in \fP(\fT(\fu))$ with $\scI(\fp') =
        L'$, $d_{\fp'}(\fcc(\fp'), \fcc(\fu)) < 4$ and $9\fp'' \lesssim 9\fp'$.
        For then, let $\fq \in \fT(\fu)$ with $L \cap E(\fq) \ne \emptyset$. As shown above, this
        implies $\ps(\fq) \ge s(L')$, so by \eqref{dyadicproperty} $L' \subset \scI(\fq)$.
        If $q \in B_{\fq}(\fcc(\fq), 1)$, then by a similar calculation as above, using the triangle
        inequality, \Cref{monotone-cube-metrics} and \eqref{forest1}, we obtain
        $$
            d_{\fp'}(\fcc(\fp'), q) \le d_{\fp'}(\fcc(\fp'), \fcc(\fq)) + d_{\fq}(\fcc(\fq), q) \le
            9\,.
        $$
        Thus $L \cap G \cap E(\fq) \subset E_2(9, \fp')$. We deduce using the definition
        \eqref{definedens1} of $\dens_1$
        $$
            \mu(L \cap G \cap \bigcup_{\fq \in \fT(\fu)} E(\fq)) \le \mu(E_2(9, \fp')) \le 9^a
            \dens_1(\fT(\fu)) \mu(L')\,.
        $$
        Using the doubling property \eqref{doublingx}, \eqref{eq-vol-sp-cube}, and $a \ge 4$ this is
        estimated by
        $$
            9^a 2^{100a^3 + 5a}\dens_1(\fT(\fu)) \mu(L) \le 2^{101 a^3} \dens_1(\fT(\fu))\mu(L)\,.
        $$
        To show existence of $\fp'$ with the given properties, if $\scI(\fp'') = L'$ we can take
        $\fp' = \fp''$, which satisfies the distance property by \eqref{forest1} and the other
        properties trivially.
        Otherwise, let $\fp'$ be the unique tile such that $\scI(\fp') = L'$ and such that
        $\Omega(\fu) \cap \Omega(\fp') \ne \emptyset$.
        Since $\scI(\fp') \subset \scI(\fp)$ and $\fp \in \fT(\fu)$, we have $\fp' \in
        \fP(\fT(\fu))$.
        Since by \eqref{forest1} $\ps(\fp') = s(L') \le \ps(\fp) < \ps(\fu)$, we have by
        \eqref{dyadicproperty} and \eqref{eq-freq-dyadic} that $\Omega(\fu) \subset \Omega(\fp')$,
        and hence the distance property.
        $9\fp'' \lesssim 9\fp'$ follows by the triangle inequality, \eqref{forest1},
        \Cref{monotone-cube-metrics} and \eqref{eq-freq-comp-ball}.
...
@[blueprint
  ...
  (title := "local dens2 tree bound")
  (statement := /-- Let $\fu \in \fU$ and $J \in \mathcal{J}(\fT(\fu))$. Then
    $$
        \mu(F \cap J) \le 2^{201a^3} \dens_2(\fT(\fu)) \mu(J)\,.
    $$ -/)
  (proof := /-- [Proof of \Cref{local-dens2-tree-bound}]
    
    
        We prove the inequality with the constant $2^{201a^3}$ replaced by $2 ^ {200a^3 + 14a}$;
        this
        is stronger because $a \geq 4$. It suffices to show the existence of a tile $\fp \in
        \fT(\fu)$
        and an $r \geq 4 D ^ {\ps(\fp)}$ such that $J \subset B(\pc(\fp), r)$ and
        $\mu(B(\pc(\fp), r)) ≤ 2 ^ {200a^3 + 14a} \mu(J)$, because then it follows from the
        definition
        ...
        $$
        $$
            \le \dens_2(\fT(\fu)) \mu(B(\pc(\fp), r)) \le 2^{200a^3 + 14a}
            \dens_2(\fT(\fu))\mu(J)\,.
        $$
        ...
        \eqref{dyadicproperty}, there exists some cube $J' \in \mathcal{D}$ with $s(J') = s(J) + 1$
        and
        $J \subset J'$. By definition of $\mathcal{J}(\fT(\fu))$ there exists some $\fp \in
        \fT(\fu)$
...
@[blueprint
  ...
  (title := "densities tree bound")
  (statement := /-- Let $\fu \in \fU$. Then for all bounded $f$ with bounded support and bounded $g$
    supported on $G$ we have
    \begin{equation}
        \label{eq-cor-tree-est}
        \left|\int_X \bar g \sum_{\fp \in \fT(\fu)} T_{\fp }f \, \mathrm{d}\mu \right| \le
        2^{181a^3} \dens_1(\fT(\fu))^{1/2} \|f\|_2\|g\|_2\,.
    \end{equation}
    If additionally $\text{support}(f) \subseteq F$, then we have
    \begin{equation}
        \label{eq-cor-tree-est-F}
        \left| \int_X \bar g \sum_{\fp \in \fT(\fu)} T_{\fp }f\, \mathrm{d}\mu \right| \le
        2^{282a^3} \dens_1(\fT(\fu))^{1/2} \dens_2(\fT(\fu))^{1/2} \|f\|_2\|g\|_2\,.
    \end{equation} -/)
  (proof := /-- [Proof of \Cref{densities-tree-bound}]
    
    
        % First, we claim that for every tile $\fp \in \fT(\fu)$
        % we have
        % \[
        %     T_{\fp} f = \mathbf{1}_{E(\fp)}(x) \int K_{\ps(\fp)}(x,y) f(y)
        % e(\tQ(x)(y)-\tQ(x)(x))\, d\mu(y)
        % \]
        % \[
        ...
        Then we have
        $$
            \left| \int_X \bar g \sum_{\fp \in \fT(\fu)} T_{\fp} f \, \mathrm{d}\mu \right| = \left|
            \int_X \overline{ g\mathbf{1}_{\mathcal{E}(\fu)}} \sum_{\fp \in \fT(\fu)} T_{\fp}
            (\mathbf{1}_{\scI(\fu)}f) \, \mathrm{d}\mu \right|\,.
        $$
        By \Cref{tree-projection-estimate}, this is bounded by
        \begin{equation}
            \label{eq-both-factors-tree}
            \le 2^{130a^3}\|P_{\mathcal{J}(\fT(\fu))}|f|\|_2 \|P_{\mathcal{L}(\fT(\fu))}
            |\mathbf{1}_{\mathcal{E}(\fu)}g|\|_2\,.
        \end{equation}
        We bound the two factors separately.
        We have
        $$
            \|P_{\mathcal{L}(\fT(\fu))} |\mathbf{1}_{\mathcal{E}(\fu)}g|\|_2 = \left( \sum_{L \in
            \mathcal{L}(\fT(\fu))} \frac{1}{\mu(L)} \left(\int_{L \cap \mathcal{E}(\fu)} |g(y)| \,
            \mathrm{d}\mu(y)\right)^2 \right)^{1/2}\,.
        $$
        By Cauchy-Schwarz and \Cref{local-dens1-tree-bound} this is at most
        $$
            \le \left( \sum_{L \in \mathcal{L}(\fT(\fu))} 2^{101a^3} \dens_1(\fT(\fu)) \int_{L \cap
            \mathcal{E}(\fu)} |g(y)|^2 \, \mathrm{d}\mu(y) \right)^{1/2}\,.
        $$
        Since cubes $L \in \mathcal{L}(\fT(\fu))$ are pairwise disjoint by \Cref{dyadic-partitions},
        this is
        \begin{equation}
        ...
        \begin{equation}
            ...
            \|P_{\mathcal{J}(\fT(\fu))}|f|\|_2 = \left( \sum_{J \in \mathcal{J}(\fT(\fu))}
            \frac{1}{\mu(J)} \left(\int_J |f(y)| \, \mathrm{d}\mu(y)\right)^2 \right)^{1/2}\,.
        \end{equation}
        By Cauchy-Schwarz, this is
        $$
            \le \left( \sum_{J \in \mathcal{J}(\fT(\fu))} \int_J |f(y)|^2 \, \mathrm{d}\mu(y)
            \right)^{1/2}\,.
        $$
        Since cubes in $\mathcal{J}(\fT(\fu))$ are pairwise disjoint by \Cref{dyadic-partitions},
        this is at most
        \begin{equation}
            \label{eq-factor-J-tree}
            \|f\|_2\,.
        \end{equation}
        Combining \eqref{eq-both-factors-tree}, \eqref{eq-factor-L-tree} and
        \eqref{eq-factor-J-tree} gives \eqref{eq-cor-tree-est}.
    
        If $f \le \mathbf{1}_F$ then $f = f\mathbf{1}_F$, so
        $$
            \left( \sum_{J \in \mathcal{J}(\fT(\fu))} \frac{1}{\mu(J)} \left(\int_J |f(y)| \,
            \mathrm{d}\mu(y)\right)^2 \right)^{1/2}
        $$
        $$
            =\left(\sum_{J \in \mathcal{J}(\fT(\fu))} \frac{1}{\mu(J)} \left(\int_{J \cap F} |f(y)|
            \, \mathrm{d}\mu(y)\right)^2 \right)^{1/2}\,.
        $$
        We estimate as before, using now \Cref{local-dens2-tree-bound} and Cauchy-Schwarz, and
        obtain that this is
        $$
            \le 2^{101a^3} \dens_2(\fT(\fu))^{1/2} \|f\|_2\,.
        $$
        Combining this with \eqref{eq-both-factors-tree} and \eqref{eq-factor-L-tree} gives
        \eqref{eq-cor-tree-est-F}. -/)
  (latexEnv := "lemma")]
